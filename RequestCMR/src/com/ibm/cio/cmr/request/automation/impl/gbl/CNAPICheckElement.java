...
//        Map<String, IntlAddr> noSoldToAddrListMap = new HashMap<String, IntlAddr>();
...
//        }
      ...
      for (IntlAddrRdc intlAddrRdc : addrRdcList) {
        ...
        if (intlAddr.getId().getAddrSeq().equals(intlAddrRdc.getId().getAddrSeq())) {
          ...
          if (!(cnAddressAddr.equals(cnAddressAddrRdc))) {
            result.setResults("Review Needed");
            details.append("The Chinese address of the non-SOLDTO address type is changed from " + cnAddressAddrRdc + " to " + cnAddressAddr).append("\n");                    
            result.setOnError(true);
          }
          if (result.isOnError()) {
  ...
  private void checkChineseName(IntlAddr soldToIntlAddr, IntlAddrRdc soldToIntlAddrRdc, CompanyRecordModel searchModel, AutomationResponse<CNResponse> cmrsData, 
...
//      result.setOnError(false);
  ...
  private void checkChineseAddress(IntlAddr soldToIntlAddr, IntlAddrRdc soldToIntlAddrRdc, CompanyRecordModel searchModel, AutomationResponse<CNResponse> cmrsData, 
      AutomationResult<ValidationOutput> result, StringBuilder details, AutomationEngineData engineData, EntityManager entityManager) {
