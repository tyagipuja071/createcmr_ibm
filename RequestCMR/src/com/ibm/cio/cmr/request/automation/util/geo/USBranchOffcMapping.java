...
public class USBranchOffcMapping {
  ...
  public String getCsoSiteForRevivedCMRs(String scenario) throws CmrException {
    String calculatedCsoSite = StringUtils.isBlank(csoSite) ? "" : csoSite;
    if ("logic".equalsIgnoreCase(csoSite)) {
      calculatedCsoSite = "";
    }
    return calculatedCsoSite;
  }

  public String getPccArDeptForRevivedCMRs(EntityManager entityManager, String scenario, RevivedCMRModel revCmrModel) throws Exception {
    String calculatedPccArDept = "";
    // get request admin and data
    switch (pccArDept) {
    // case REQUEST:
    // calculatedPccArDept = StringUtils.isBlank(data.getPccArDept()) ? "" :
    // data.getPccArDept();
    // break;
    case LOGIC:
      if (USUtil.SC_BP_END_USER.equals(scenario) || USUtil.SC_BP_POOL.equals(scenario) || USUtil.SC_BP_DEVELOP.equals(scenario)
        ...
        String mktArBo = getMtkgArDeptForRevivedCMRs(scenario);
      ...
      break;
    default:
      if (USUtil.SC_BYMODEL.equals(scenario)
          && !(USUtil.SC_FED_REGULAR.equals(scenario) || USUtil.SC_FED_POA.equals(scenario) || USUtil.SC_REST_OIO.equals(scenario))) {
        USDetailsContainer usDetails = USUtil.determineUSCMRDetails(entityManager, revCmrModel.getCmrNo());
  ...
  public String getMtkgArDeptForRevivedCMRs(String scenario) throws Exception {
    String calculatedMtkgArDept = "";
    switch (mtkgArDept) {
    // case REQUEST:
    // calculatedMtkgArDept = StringUtils.isBlank(data.getMtkgArDept()) ? "" :
    // data.getMtkgArDept();
    // break;
    case LOGIC:
      ...
      if (USUtil.SC_FED_POA.equals(scenario)) {
        ...
        // LOG.debug("Getting Marketing A/R BO with highest CMR count belonging
        // to the enterprise=" + data.getEnterprise());
        // String url = SystemConfiguration.getValue("CMR_SERVICES_URL");
        // String usSchema = SystemConfiguration.getValue("US_CMR_SCHEMA");
        // String sql = ExternalizedQuery.getSql("AUTO.GET_MKTG_AR_DEPT_USCMR",
        // usSchema);
        // sql = StringUtils.replace(sql, ":ENTERPRISE", "'" +
        // (StringUtils.isNotBlank(data.getEnterprise()) ? data.getEnterprise()
        // : "") + "'");
        ...
        // QueryClient.class);
        // QueryResponse response = client.executeAndWrap(dbId, query,
        // QueryResponse.class);
        ...
        // LOG.debug("Marketing A/R BO Blank. Calculating using Sub Industry
        // Mapping");
        // String indToMatch = StringUtils.isBlank(data.getSubIndustryCd()) ? ""
        // : data.getSubIndustryCd().substring(0, 1);
        ...
        // industryList.contains(indToMatch)) {
  ...
  public String getMktgDeptForRevivedCMRs(String scenario, RevivedCMRModel revCmrModel) throws Exception {
    String calculatedMktgDept = "";
    String stateToMatch = "";
    String countyToMatch = "";
    stateToMatch = StringUtils.isBlank(revCmrModel.getCmrState()) ? "" : revCmrModel.getCmrState();
    countyToMatch = StringUtils.isBlank(revCmrModel.getCounty()) ? "" : revCmrModel.getCounty();

    switch (mktgDept) {
    // case REQUEST:
    // calculatedMktgDept = StringUtils.isBlank(data.getMktgDept()) ? "" :
    // data.getMktgDept();
    // break;
    case LOGIC:
  ...
  public String getSvcArOfficeForRevivedCMRs(String scenario) throws Exception {
    String calculatedSvcArOffice = "";
    switch (svcArOffice) {
    // case REQUEST:
    // calculatedSvcArOffice = StringUtils.isBlank(data.getSvcArOffice()) ? "" :
    // data.getSvcArOffice();
    // break;
    case LOGIC:
      if (!USUtil.svcARBOMappings.isEmpty()) {
        String mtkgArDept = getMtkgArDeptForRevivedCMRs(scenario);
