...
public abstract class USBPHandler {
	...
	public static final List<String> DEPARTMENT_IDENTIFIERS = Arrays.asList("E-HOST", "EHOST", "DEMO DEVELOPMENT",
			"DEMO DEV", "DEVELOPMENT", "ICC LEASE DEV", "IDL LEASE DEV", "POOL");
	...
	public static USBPHandler getBPHandler(EntityManager entityManager, RequestData requestData,
		...
		if (USUtil.CG_THIRD_P_BUSINESS_PARTNER.equals(custGrp)) {
			...
			if (USUtil.BUSINESS_PARTNER.equals(type)) {
				if ("E".equals(data.getBpAcctTyp()) && (RESTRICT_TO_END_USER.equals(data.getRestrictTo())
						|| RESTRICT_TO_MAINTENANCE.equals(data.getRestrictTo()))) {
					...
					return new USBPPoolHandler();
				} else if (RESTRICT_TO_END_USER.equals(data.getRestrictTo()) && (deptAttn.contains("demo dev")
						|| deptAttn.contains("lease dev") || deptAttn.contains("development"))) {
					return new USBPDevelopHandler();
				}
	 ...
	 * @return
	 */
	public abstract boolean doInitialValidations(Admin admin, Data data, Addr addr,
			AutomationResult<OverrideOutput> output, AutomationEngineData engineData);
	 ...
	 * @throws Exception
	 */
	public abstract boolean processRequest(EntityManager entityManager, RequestData requestData,
			AutomationEngineData engineData, AutomationResult<OverrideOutput> output, StringBuilder details,
			boolean childCompleted, RequestData childRequest, GEOHandler handler, FindCMRRecordModel ibmCmr,
	 ...
	 * @param ibmCmr
	 */
	public abstract void copyAndFillIBMData(EntityManager entityManager, GEOHandler handler, RequestData requestData,
			AutomationEngineData engineData, StringBuilder details, OverrideOutput overrides, RequestData childRequest,
			FindCMRRecordModel ibmCmr);
	 ...
	 * @param result
	 */
	public abstract void doFinalValidations(AutomationEngineData engineData, RequestData requestData,
			StringBuilder details, OverrideOutput overrides, FindCMRRecordModel ibmCmr,
	 ...
	 * @throws Exception
	 */
	public void matchAgainstSosRpa(GEOHandler handler, RequestData requestData, Addr addr,
			AutomationEngineData engineData, StringBuilder details, OverrideOutput overrides, boolean hasExistingCmr)
			throws Exception {
		...
		for (String addrType : RELEVANT_ADDRESSES) {
			...
			if (!sosRpaMatches.isEmpty()) {
				...
				details.append("\nCompany Id = "
						+ (StringUtils.isBlank(response.getCompanyId()) ? "" : response.getCompanyId()));
				details.append("\nCustomer Name = "
						+ (StringUtils.isBlank(response.getLegalName()) ? "" : response.getLegalName()));
				details.append(
				...
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ADMN", "COMP_VERIFIED_INDC",
						requestData.getAdmin().getCompVerifiedIndc(), "Y");
			} else {
				if (itr == 1 && ("N".equals(scorecard.getRpaMatchingResult())
						|| "".equals(scorecard.getRpaMatchingResult()))) {
	 ...
	 * @throws Exception
	 */
	public DnBMatchingResponse matchAgainstDnB(GEOHandler handler, RequestData requestData, Addr addr,
			AutomationEngineData engineData, StringBuilder details, OverrideOutput overrides, boolean hasExistingCmr)
			throws Exception {
		Scorecard scorecard = requestData.getScorecard();
	 ...
	 * @param childRequest
	 */
	public void createAddressOverrides(EntityManager entityManager, GEOHandler handler, RequestData requestData,
			AutomationEngineData engineData, StringBuilder details, OverrideOutput overrides, RequestData childRequest,
			FindCMRRecordModel ibmCmr) {
		...
		if (createAddressOverrides) {
			...
			if (!StringUtils.equals(installAt.getDivn(), mainCustNm1)
					|| !StringUtils.equals(installAt.getDept(), mainCustNm2)) {

				String customerName = installAt.getDivn()
						+ (!StringUtils.isBlank(installAt.getDept()) ? " " + installAt.getDept() : "");
				...
				if (customerName.contains("CHN SVCS")) {
					...
					customerNameSuffix = customerName.substring(customerName.lastIndexOf("CHANNEL SVCS USE ONLY"))
							.trim();
				...
				nameParts[1] = (nameParts[1] != null ? nameParts[1] : "")
						+ (StringUtils.isBlank(customerNameSuffix) ? "" : " " + customerNameSuffix);
				...
				details.append(
						"Updated Division to " + (StringUtils.isNotBlank(nameParts[0]) ? nameParts[0] : "-blank-"))
						.append("\n");
				...
				if (useDeptForMatching(requestData)) {
					...
					details.append("Updated Department to "
							+ (StringUtils.isNotBlank(nameParts[1]) ? nameParts[1] : "-blank-")).append("\n");
				}
			}
			...
			if (!StringUtils.equals(installAt.getAddrTxt(), streetAddress1)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "ADDR_TXT",
						installAt.getAddrTxt(), StringUtils.isNotBlank(streetAddress1) ? streetAddress1.trim() : "");
				details.append("Updated Street Address1 to "
						+ (StringUtils.isNotBlank(streetAddress1) ? streetAddress1.trim() : "-blank-")).append("\n");
			}
			if (!StringUtils.equals(installAt.getAddrTxt2(), streetAddress2)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "ADDR_TXT_2",
						installAt.getAddrTxt2(), StringUtils.isNotBlank(streetAddress2) ? streetAddress2.trim() : "");
				details.append("Updated Street Address2 to "
						+ (StringUtils.isNotBlank(streetAddress2) ? streetAddress2.trim() : "-blank-")).append("\n");
			}
			if (!StringUtils.equals(installAt.getCity1(), city1)) {
				...
				details.append("Updated City to " + (StringUtils.isNotBlank(city1) ? city1.trim() : "-blank-"))
						.append("\n");
			}
			if (!StringUtils.equals(installAt.getPostCd(), postalCd)) {
				...
				details.append(
						"Updated Postal Code to " + (StringUtils.isNotBlank(postalCd) ? postalCd.trim() : "-blank-"))
						.append("\n");
			}
			if (!StringUtils.equals(installAt.getStateProv(), stateProv)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "STATE_PROV",
						installAt.getStateProv(), StringUtils.isNotBlank(stateProv) ? stateProv.trim() : "");
				details.append(
						"Updated State/Prov to " + (StringUtils.isNotBlank(stateProv) ? stateProv.trim() : "-blank-"))
						.append("\n");
			}
			...
			if (!StringUtils.equals(installAt.getCounty(), county)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "COUNTY_NAME",
						installAt.getCountyName(), StringUtils.isNotBlank(countyNm) ? countyNm.trim() : "");
				details.append("Updated County to " + (StringUtils.isNotBlank(countyNm) ? countyNm.trim() : "-blank-"))
						.append("\n");
			}

			if (!StringUtils.equals(installAt.getCustPhone(), phone)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "CUST_PHONE",
						installAt.getCustPhone(), StringUtils.isNotBlank(phone) ? phone.trim() : "");
			...
			if (!StringUtils.equals(installAt.getCustFax(), fax)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "CUST_FAX",
						installAt.getCustFax(), StringUtils.isNotBlank(fax) ? fax.trim() : "");
			...
			if (!StringUtils.equals(installAt.getTransportZone(), transportZone)) {
				overrides.addOverride(AutomationElementRegistry.US_BP_PROCESS, "ZS01", "TRANSPORT_ZONE",
						installAt.getTransportZone(),
	...
	public FindCMRRecordModel createIBMCMRFromChild(RequestData childRequest) {
		...
		boolean federalPoa = childIsic != null
				&& (childIsic.startsWith("90") || childIsic.startsWith("91") || childIsic.startsWith("92"));
		if (federalPoa) {
		...
		ibmCmrModel.setCmrStreetAddress(childInstallAt.getAddrTxt()
				+ (StringUtils.isNotBlank(childInstallAt.getAddrTxt2()) ? childInstallAt.getAddrTxt2() : ""));
	 ...
	 */
	public FindCMRRecordModel findIBMCMR(EntityManager entityManager, GEOHandler handler, RequestData requestData,
			Addr addr, AutomationEngineData engineData, String directCmrNo) throws Exception {
		...
		if (!StringUtils.isBlank(directCmrNo)) {
			...
			String customerName = addr.getDivn()
					+ ((!StringUtils.isBlank(addr.getDept()) && useDeptForMatching(requestData)) ? " " + addr.getDept()
							: "");
			customerName = customerName.toUpperCase();
			if (customerName.contains("CHN SVCS")) {
			...
			if (res != null && res.getSuccess() && res.getMatched()) {
				LOG.debug("Matches found for request " + requestData.getAdmin().getId().getReqId()
						+ " with customer Name " + customerName);
				...
				LOG.debug("No matches found for request " + requestData.getAdmin().getId().getReqId()
						+ " with customer Name " + customerName);
			}
		}
	...
	protected void tweakFindCMRRequest(EntityManager entityManager, GEOHandler handler, RequestData requestData,
	 ...
	 * @throws CmrException
	 */
	public void processChildCancellation(EntityManager entityManager, StringBuilder details, RequestData requestData,
			RequestData childRequestData, AutomationEngineData engineData, WfHist rejectionHist)
		...
		if (rejectionHist != null) {
			...
			String rejectCmt = StringUtils.isBlank(rejectionHist.getCmt())
					? "Request rejected by processor. Please check comments." : rejectionHist.getCmt();
			...
			engineData.addRejectionComment(rejectionHist.getRejReasonCd(), rejectCmt, rejectionHist.getRejSupplInfo1(),
					rejectionHist.getRejSupplInfo2());
			...
			if (cmts != null) {
				for (ReqCmtLog cmt : cmts) {
					RequestUtils.createCommentLogFromBatch(entityManager, cmt.getCreateById(),
							requestData.getAdmin().getId().getReqId(), cmt.getCmt());
				}
			}
					...
					for (Attachment attachment : attachments) {
						...
						if (file.exists()) {
							try (FileInputStream fis = new FileInputStream(file)) {
								...
								attachService.addExternalAttachment(entityManager, user, parentReqId,
										attachment.getDocContent(), file.getName(), attachment.getCmt(), fis);
							}
						}
					}
				}
	 ...
	 * @throws Exception
	 */
	public boolean checkIfChildRequestCompleted(EntityManager entityManager, AutomationEngineData engineData,
			RequestData requestData, RequestData childRequest, StringBuilder details,
			AutomationResult<OverrideOutput> output, GEOHandler handler) throws Exception {
		...
		if ("PRJ".equals(childReqStatus) || "DRA".equals(childReqStatus) || "CAN".endsWith(childReqStatus)) {
				...
				if (rejectHist != null) {
					...
					if ("DUPC".equals(rejectHist.getRejReasonCd())) {
						...
						if (!StringUtils.isBlank(rejectionCmrNo)) {
							details.append("Child CMR Rejected and specified IBM CMR No. :" + rejectionCmrNo)
									.append("\n");
						}
					}
				}
			...
			processChildCancellation(entityManager, details, requestData, childRequest, engineData,
					setError ? rejectHist : null);

			if (setError) {
				LOG.debug(
						"Child request processing has been stopped (Rejected or Sent back to requesters). Returning error.");
				...
				LOG.debug("Child Request was not completed, but CMR " + this.ibmCmr.getCmrNum() + "("
						+ this.ibmCmr.getCmrName() + ") was found.");
				details.append("Child Request was not completed, but CMR " + this.ibmCmr.getCmrNum() + "("
						+ this.ibmCmr.getCmrName() + ") was found.\n");
				return true;
			}
	 ...
	 * @throws CmrException
	 */
	protected long createChildRequest(EntityManager entityManager, RequestData requestData,
			AutomationEngineData engineData) throws CmrException {
	...
	/**
	 * Adjusts the child request data using the logic for BP creation. Creates
	 * an internal 'details' on the information that needs to be added on the
	 * processing
	 * 
	 * @param entityManager
	 * @param requestData
	 ...
	 * @throws Exception
	 */
	protected String completeChildRequestDataAndAddress(EntityManager entityManager, RequestData requestData,
			AutomationEngineData engineData, long childReqId, DnBMatchingResponse dnbMatch) throws Exception {
		...
		if (USUtil.CG_THIRD_P_BUSINESS_PARTNER.equals(childData.getCustGrp())
				&& USUtil.SC_BP_POOL.equals(childData.getCustSubGrp())) {
		...
		if (dnbMatch != null) {
			...
			DnBCompany dnbCompany = dnbData != null && dnbData.getResults() != null && !dnbData.getResults().isEmpty()
					? dnbData.getResults().get(0) : null;
			if (dnbCompany != null) {
				if (dnbCompany.getIbmIsic() != null) {
					childData.setIsicCd(dnbCompany.getIbmIsic());
					childData.setUsSicmen(dnbCompany.getIbmIsic());
					String subInd = RequestUtils.getSubIndustryCd(entityManager, dnbCompany.getIbmIsic(),
							SystemLocation.UNITED_STATES);
		...
		RequestUtils.createWorkflowHistoryFromBatch(entityManager, "AutomationEngine", childAdmin,
				"Sending for processing via Automation Engine", "Send for Processing", procCenter, procCenter, false,
				false, null);
	 ...
	 * @param details
	 */
	protected void modifyChildDataValues(EntityManager entityManager, RequestData requestData, RequestData childReqData,
			StringBuilder details) {
		// NOOP
	}

	/**
	 * Sets the correct scenario on the child record based on request
	 * information
	 * 
	 * @param data
	 * @param childData
	...
	public FindCMRRecordModel getIbmCmr(EntityManager entityManager, GEOHandler handler, RequestData requestData,
			StringBuilder details, Addr zs01, AutomationEngineData engineData, RequestData childRequest,
			boolean childCompleted) throws Exception {
		if (this.ibmCmr == null) {
			...
			if (childCompleted) {
				details.append("Copying CMR values direct CMR " + childCmrNo + " from Child Request "
						+ childRequest.getAdmin().getId().getReqId() + ".\n");
