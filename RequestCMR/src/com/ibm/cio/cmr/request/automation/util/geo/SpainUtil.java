...
public class SpainUtil extends AutomationUtil {
	...
	private static final List<String> RELEVANT_ADDRESSES = Arrays.asList(CmrConstants.RDC_SOLD_TO,
			...
			CmrConstants.RDC_SECONDARY_SOLD_TO);
	...
	private static final List<String> SCENARIOS_TO_SKIP_COVERAGE = Arrays.asList(SCENARIO_INTERNAL,
			SCENARIO_INTERNAL_SO, SCENARIO_BUSINESS_PARTNER, SCENARIO_CROSSBORDER_BP);
	...
	public boolean performScenarioValidation(EntityManager entityManager, RequestData requestData,
		...
		if ("C".equals(requestData.getAdmin().getReqType())) {
			if ((SCENARIO_COMMERCIAL.equals(scenario) || SCENARIO_IGS_GSE.equals(scenario)
					|| SCENARIO_CROSSBORDER.equals(scenario) || SCENARIO_CROSSBORDER_IGS.equals(scenario)
					|| SCENARIO_GOVERNMENT.equals(scenario) || SCENARIO_GOVERNMENT_IGS.equals(scenario))
				...
				details.append(
						"Customer names on billing and installing address are not identical, 3rd Party should be selected.");
				return false;
			} else if ((SCENARIO_THIRD_PARTY.equals(scenario) || SCENARIO_THIRD_PARTY_IG.equals(scenario))
					&& addressEquals(soldTo, installAt)) {
		...
		case SCENARIO_IBM_EMPLOYEE:
			...
			if (StringUtils.isNotBlank(soldTo.getCustNm1())) {
				try {
					String mainCustName = soldTo.getCustNm1()
							+ (StringUtils.isNotBlank(soldTo.getCustNm2()) ? " " + soldTo.getCustNm2() : "");
	...
	public AutomationResult<OverrideOutput> doCountryFieldComputations(EntityManager entityManager,
		...
		if (SCENARIO_THIRD_PARTY.equals(scenario) || SCENARIO_THIRD_PARTY_IG.equals(scenario)) {
					...
					for (DnBMatchingResponse dnbRecord : dnbMatches) {
						...
						if (closelyMatches) {
							...
							if (dnbData != null) {
								...
								details.append(
										"ISIC =  " + dnbData.getIbmIsic() + " (" + dnbData.getIbmIsicDesc() + ")")
										.append("\n");
			...
			details.append("Ensuring ISIC is set to '0000' for "
					+ (SCENARIO_INTERNAL.equals(scenario) ? "Internal" : "Internal SO") + "scenario.");
			overrides.addOverride(AutomationElementRegistry.GBL_FIELD_COMPUTE, "DATA", "ISIC_CD", data.getIsicCd(),
					"0000");
	...
	protected List<String> getCountryLegalEndings() {
		return Arrays.asList("SL", "S.L.", "S.A.", "SLL", "SA", "LTD", "SOCIEDAD LIMITADA", "SLP", "S.C.C.L.", "SLU",
				"SAU", "S.A.U", "C.B.", "S.E.E.");
	}
	...
	public boolean runUpdateChecksForData(EntityManager entityManager, AutomationEngineData engineData,
		...
		for (UpdatedDataModel change : changes.getDataUpdates()) {
			...
			case "VAT #":
				if (requestData.getAddress("ZS01").getLandCntry().equals("GB")) {
					...
					if (StringUtils.isBlank(change.getOldData()) && !StringUtils.isBlank(change.getNewData())) {
						...
						if (!matchesDnb) {
							cmdeReview = true;
							engineData.addNegativeCheckStatus("_esVATCheckFailed",
					...
					if (!StringUtils.isBlank(change.getOldData()) && !StringUtils.isBlank(change.getNewData())
						...
						if (!(oldData.equals(newData))) {
							...
							details.append(
									"VAT # on the request has characters updated other than the first character. Create New CMR. \n");
						} else {
			...
			case "SBO":
				if (!StringUtils.isBlank(change.getOldData()) && !StringUtils.isBlank(change.getNewData())
						&& !(change.getOldData().equals(change.getNewData()))) {
					requesterFromTeam = BluePagesHelper.isBluePagesHeirarchyManager(admin.getRequesterId(),
							SystemParameters.getList("ES.SKIP_UPDATE_CHECK"));
					if ("9".equals(change.getNewData().substring(1, 2)) && !requesterFromTeam) {
						resultCodes.add("D");// Reject
						details.append("Requester is not allowed to submit updates to " + change.getDataField()
								+ " field. \n");
					}
			...
			case "Mode Of Payment":
				requesterFromTeam = BluePagesHelper.isBluePagesHeirarchyManager(admin.getRequesterId(),
						SystemParameters.getList("ES_MOPA_AUTO_UPDATE"));
				if (requesterFromTeam) {
					details.append(
							"Skipping validation for MOPA update for requester - " + admin.getRequesterId() + ".\n");
			...
			case "Mailing Condition":
				...
				if (!requesterFromTeam) {
					resultCodes.add("D");// Reject
					details.append(
							"Requester is not allowed to submit updates to " + change.getDataField() + " field. \n");
				}
				break;
			case "Tax Code":
	...
	public boolean runUpdateChecksForAddress(EntityManager entityManager, AutomationEngineData engineData,
		...
		int zp01count = 0;
		int zd01count = 0;
		List<Integer> addrCount = getAddressCount(entityManager, SystemLocation.SPAIN, data.getCmrIssuingCntry(),
				data.getCmrNo());
				...
				for (Addr addr : addresses) {
					if ("N".equals(addr.getImportInd())) {
						...
						if (CmrConstants.RDC_BILL_TO.equals(addrType)
							...
							LOG.debug("New address " + addrType + "(" + addr.getId().getAddrSeq()
									+ ") needs to be verified");
							resultCodes.add("R");
							checkDetails.append("New address " + addrType + "(" + addr.getId().getAddrSeq()
									+ ") needs to be verified \n");
						}
						...
						if ((CmrConstants.RDC_INSTALL_AT.equals(addrType)
							...
							if (CmrConstants.RDC_SHIP_TO.equals(addrType)
								...
								LOG.debug("Update to Shipping and EPL " + addrType + "(" + addr.getId().getAddrSeq()
										+ ")");
								checkDetails.append("Updates to Shipping and EPL (" + addr.getId().getAddrSeq()
										+ ") skipped in the checks.\n");
							} else {
								checkDetails.append("Updates to Updated Addresses for " + addrType + "("
										+ addr.getId().getAddrSeq() + ") needs to be verified").append("\n");
								resultCodes.add("R");
							}
						}
					}
	...
	public boolean performCountrySpecificCoverageCalculations(CalculateCoverageElement covElement,
		...
		if (bgId != null && !"BGNONE".equals(bgId.trim())) {
			List<SpainFieldsContainer> enterpriseResults = computeSREnterpriseES(entityManager, QUERY_BG_ES, bgId,
					data.getCmrIssuingCntry(), false);
			if (enterpriseResults != null && !enterpriseResults.isEmpty()) {
				for (SpainFieldsContainer field : enterpriseResults) {
					String containerCtc = StringUtils.isBlank(container.getClientTierCd()) ? ""
							: container.getClientTierCd();
					String calculatedCtc = StringUtils.isBlank(field.getClientTier()) ? "" : field.getClientTier();
					if (field.getIsuCd().equals(container.getIsuCd()) && containerCtc.equals(calculatedCtc)
							&& !StringUtils.isBlank(field.getEnterprise())) {
		...
		if (!StringUtils.isBlank(enterprise) && !StringUtils.isBlank(salesRep)) {
			...
			details.append("Enterprise : " + (StringUtils.isBlank(enterprise) ? data.getEnterprise() : enterprise))
					.append("\n");
			overrides.addOverride(AutomationElementRegistry.GBL_CALC_COV, "DATA", "ENTERPRISE", data.getEnterprise(),
					enterprise);
			overrides.addOverride(AutomationElementRegistry.GBL_CALC_COV, "DATA", "REP_TEAM_MEMBER_NO",
					data.getRepTeamMemberNo(), salesRep);
			results.setResults("Calculated");
			results.setDetails(details.toString());
			...
			details.append("Coverage could not be calculated from the found CMRs. Using values from request")
					.append("\n");
	...
	private HashMap<String, String> getEntpSalRepFromPostalCodeMapping(String subIndCd, Addr addr, String isuCd,
			String clientTier, String scenario) {
			...
			if (esIsicPostalMapping != null) {
				...
				for (ESPostalMapping postalMapping : postalMappings) {
					if (esIsicPostalMapping.getSubIndCds() != null && !esIsicPostalMapping.getSubIndCds().isEmpty()) {
						isicCds = Arrays.asList(
								esIsicPostalMapping.getSubIndCds().replaceAll("\n", "").replaceAll(" ", "").split(","));
					}
					...
					if (isuCd.concat(clientTier).equalsIgnoreCase(postalMapping.getIsuCTC())
							&& scenariosList.contains(scenario)
							&& ("None".equalsIgnoreCase(postalMapping.getIsicBelongs())
									|| ((!postalCodes.isEmpty() && postalCodes.contains(postCdtStrt))
											&& (("Yes".equalsIgnoreCase(postalMapping.getIsicBelongs())
													&& isicCds.contains(subIndCd)) || ("No"
															.equalsIgnoreCase(postalMapping.getIsicBelongs())
															&& !isicCds.contains(subIndCd)))))) {
	...
	private List<SpainFieldsContainer> computeSREnterpriseES(EntityManager entityManager, String queryBgEs, String bgId,
			String cmrIssuingCntry, boolean b) {
		...
		if (results != null && !results.isEmpty()) {
			for (Object[] result : results) {
				...
				fieldValues.setSalesRep("1FICTI"); // Default Sales Rep for
													// Spain
				if (!StringUtils.isBlank((String) result[3]) && AutomationUtil.validateLOVVal(entityManager,
						cmrIssuingCntry, "##SalRepNameNo", ((String) result[3]).substring(4))) {
