...
public class SingaporeUtil extends AutomationUtil {
    ...
    if ("SPOFF".equalsIgnoreCase(data.getCustSubGrp())) {
      ...
      try {
        if (StringUtils.isNotBlank(data.getCmrIssuingCntry()) && StringUtils.isNotBlank(landCntryCd)) {
          ...
          if (cmrExists) {
            LOG.debug("CMR " + data.getCmrNo() + " exists in FIND CMR for ZS01 landed cntry of request as isusing country -> " + landCntryCd);
            if (cmrExistsFrSG) {
              ...
              if (detailsMatch) {
                ...
                details.append("CMR exists and Details Match , Hence proceeding with automating the request.");
              } else {
                LOG.debug("CMR# " + data.getCmrNo() + " Exists but request details do not match the details of CMR in FIND CMR.");
                details.append("CMR# " + data.getCmrNo() + " Exists but request details do not match the details of CMR in FIND CMR.");
  ...
  private Map<String, Boolean> checkifCMRNumExistsNDetailsMatch(String cmrNo, String landCntryCd, EntityManager entityManager, Data data, Addr addr) {
    ...
    if (cmrData != null) {
      ...
      LOG.debug("CMR No. details mentioned in the request do not exist in FIND CMR for the landed cntry > " + landCntryCd);
    }
