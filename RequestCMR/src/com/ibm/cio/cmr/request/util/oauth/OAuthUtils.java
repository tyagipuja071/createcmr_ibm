...
public class OAuthUtils {
  ...
  /**
   * This method is invoked when performing sign out activities, cleaning up
   * Http sessions and invalidating sessions. I
   */
  public static void ssoTokenCleanUP(HttpServletRequest request) {
    try {
      String access_token = (String) request.getSession().getAttribute("accessToken");
      OAuthUtils.revokeToken(access_token);
      request.logout();
    } catch (ServletException e) {
      LOG.error("SSO Token clean up was not executed successfully!");
    }

  }

  /**
   * Returns true if SSO is activated
   * 
   * @return
   */
  public static boolean isSSOActivated() {
    return Boolean.parseBoolean(SystemConfiguration.getValue("ACTIVATE_SSO"));
  }
   ...
   * @throws InvalidKeyException
   */
  public static boolean validateSignature(SimpleJWT jwt) throws InvalidKeyException, NoSuchAlgorithmException, SignatureException {
    // connect to jwks endpoint and get the Json Web Keys
    List<Map<String, String>> keys = getKeysFromAS();
  ...
  public static boolean revokeToken(String access_token) {
    if (StringUtils.isNotBlank(access_token)) {
      try {
        ...
        LOG.debug("Revoking token: " + access_token);
