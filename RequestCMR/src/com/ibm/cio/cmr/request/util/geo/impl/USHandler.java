...
public class USHandler extends GEOHandler {
	...
	@Override
	public void setDataValuesOnImport(Admin admin, Data data, FindCMRResultModel results, FindCMRRecordModel cmr)
			throws Exception {
		...
		if (retrieved) {
			...
			if ("C".equals(admin.getReqType()) && StringUtils.isNotBlank(data.getRestrictTo())
					&& "OEMHQ".equals(data.getRestrictTo())) {
	...
	private boolean queryAndAssign(String url, String sql, FindCMRResultModel model, Data data, String dbId)
			throws Exception {
		...
		if (!response.isSuccess()) {
			...
			data.setSvcTerritoryZone(
					record.get("SVC_TERRITORY_ZONE") != null ? record.get("SVC_TERRITORY_ZONE").toString() : null);
	...
	@Override
	public void setAddressValuesOnImport(Addr address, Admin admin, FindCMRRecordModel cmr, String cmrNo)
			throws Exception {
		// try US CMR DB first
		if (cmrNo != null) {
			String url = SystemConfiguration.getValue("CMR_SERVICES_URL");
	...
	public void addSummaryUpdatedFields(RequestSummaryService service, String type, String cmrCountry, Data newData,
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getRestrictTo(), newData.getRestrictTo())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getRestrictInd(), newData.getRestrictInd())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getFedSiteInd(), newData.getFedSiteInd())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getOutCityLimit(), newData.getOutCityLimit())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
			...
			update.setNewData(
					service.getCodeAndDescription(newData.getNonIbmCompanyInd(), "NonIBMCompInd", cmrCountry));
			update.setOldData(
					service.getCodeAndDescription(oldData.getNonIbmCompanyInd(), "NonIBMCompInd", cmrCountry));
			results.add(update);
		}
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getIccTaxClass(), newData.getIccTaxClass())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getIccTaxExemptStatus(), newData.getIccTaxExemptStatus())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getMiscBillCd(), newData.getMiscBillCd())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getBpAcctTyp(), newData.getBpAcctTyp())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_IBM.equals(type)
				&& !equals(oldData.getPccMktgDept(), newData.getPccMktgDept())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_IBM.equals(type)
				&& !equals(oldData.getSvcTerritoryZone(), newData.getSvcTerritoryZone())) {
			update = new UpdatedDataModel();
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getSpecialTaxCd(), newData.getSpecialTaxCd())) {
			update = new UpdatedDataModel();
	...
	public List<String> getAddressFieldsForUpdateCheck(String cmrIssuingCntry) {
		return Arrays.asList("LAND_CNTRY", "ADDR_TXT", "ADDR_TXT2", "STATE_PROV", "CITY1", "CITY2", "COUNTY", "POST_CD",
				"DIVN", "DEPT", "PO_BOX", "BLDG", "FLOOR", "CUST_PHONE", "CUST_FAX");
	}
