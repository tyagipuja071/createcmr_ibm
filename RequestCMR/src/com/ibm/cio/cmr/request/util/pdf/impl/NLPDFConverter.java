 ...
 * 
 */
public class NLPDFConverter extends DefaultPDFConverter {
	private static final Logger LOG = Logger
			.getLogger(DefaultPDFConverter.class);
	private PdfFont regularFont;

	public NLPDFConverter(String cmrIssuingCntry) throws IOException {
		super(cmrIssuingCntry);
		this.regularFont = PdfFontFactory.createFont(FontConstants.HELVETICA);
	}

	@Override
	public boolean exportToPdf(EntityManager entityManager, Admin admin,
			Data data, OutputStream os, String sysLoc) throws IOException {
		...
		try {
			...
			try {
				...
				try {
					...
					try {
						...
						document.add(createHeader("Details for Request ID "
								+ admin.getId().getReqId()));

						document.add(blankLine());
						addMainDetails(admin, data, document);

						if (Arrays.asList("CREATE", "UPDATE", "C", "U")
								.contains(admin.getReqType().toUpperCase())) {
							document.add(blankLine());
							Addr soldTo = addAddressDetails(admin, data,
							...
							addExtraSections(entityManager, admin, data,
									sysLoc, soldTo, document);

						}

						if (Arrays.asList("REACTIVATE", "DELETE", "R", "D")
								.contains(admin.getReqType().toUpperCase())) {
							document.add(blankLine());
							addDeleteReactivateDetails(admin, entityManager,
									document);
						}
			...
			LOG.error("Error in Generating PDF for Request ID "
	...
	@Override
	protected void addExtraSections(EntityManager entityManager, Admin admin,
			Data data, String sysLoc, Addr soldTo, Document document) {
	...
	protected void addMainDetails(Admin admin, Data data, Document document) {
		...
		String customerName = StringUtils.isBlank(admin.getMainCustNm1()) ? ""
				: admin.getMainCustNm1();
	...
	protected Addr addAddressDetails(Admin admin, Data data, EntityManager entityManager, Document document) {
		...
		for (Addr addr : addresses) {
			...
			if (handler.customerNamesOnAddress()) {
				address.addCell(createLabelCell("Company Name:"));
			...
			address.addCell(createLabelCell("Street+NBR:"));
			...
			lbl = PageManager.getLabel(data.getCmrIssuingCntry(), "PostalCode",
					"Postal Code");
			address.addCell(createLabelCell(lbl + ":"));
			address.addCell(createValueCell(addr.getPostCd()));

			lbl = PageManager.getLabel(data.getCmrIssuingCntry(), "City1",
					"City");
			address.addCell(createLabelCell(lbl + ":"));
			...
			if ("P".equals(dplCheck) || "F".equals(dplCheck)) {
				...
				address.addCell(createValueCell(addr.getDplChkTs() != null ? sdf
						.format(addr.getDplChkTs()) : ""));
				if ("F".equals(dplCheck)) {
	...
	protected void addConverterCustomerDetails(EntityManager entityManager, Table customer, Data data) {
		...
		customer.addCell(createLabelCell("KVK:"));
		...
		customer.addCell(createLabelCell("Economic Code:"));
		customer.addCell(createValueCell(data.getEconomicCd()));
	...
	protected void addIBMDetails(EntityManager entityManager, Data data, Document document) {
		...
		if (data.getCovId() != null) {
			ibm.addCell(createValueCell(data.getCovId()
					+ (data.getCovDesc() != null ? " - " + data.getCovDesc()
							: "")));
		} else {
		...
		if (!StringUtils.isEmpty(data.getBgId())) {
			ibm.addCell(createValueCell(data.getBgId() + " - "
					+ data.getBgDesc()));
		} else {
			ibm.addCell(createValueCell(null));
		}
	...
	protected void addConverterIBMDetails(EntityManager entityManager, Table ibm, Data data) {
		...
		ibm.addCell(createLabelCell("BO Team:"));
		ibm.addCell(createValueCell(data.getEngineeringBo()));
	}
	...
	protected Cell createValueCell(String text, int rowSpan, int colSpan) {
		...
		if ((text != null && (!text.isEmpty()))
				&& (textContainingLanguage(text) != null)) {
			try {
				...
				case "ARABIC":
					FONT = classLoader.getResource("Behdad-Regular.ttf")
							.getPath();
					font = PdfFontFactory.createFont(FONT,
							PdfEncodings.IDENTITY_H, true);
					break;
				case "RUSSIAN":
					FONT = classLoader.getResource("Lora-Regular.ttf")
							.getPath();
					font = PdfFontFactory.createFont(FONT,
							PdfEncodings.IDENTITY_H, true);
					break;
