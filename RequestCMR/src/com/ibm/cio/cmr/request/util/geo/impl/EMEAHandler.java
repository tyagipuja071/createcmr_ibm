 ...
 * 
 */
public class EMEAHandler extends BaseSOFHandler {
	...
	private final String[] NAC_VALUES = { "0915", "0918", "0977", "0988", "0992", "1071", "1076", "1082", "1090",
			"1140", "1206", "1265", "1828", "1960", "2159", "2232", "3680", "4110", "5586", "5782", "6909", "7221",
			"8820" };

	private static final String[] UKI_SKIP_ON_SUMMARY_UPDATE_FIELDS = { "Affiliate", "CAP", "CMROwner", "Company",
			"CustClassCode", "LocalTax1", "LocalTax2", "Enterprise", "SearchTerm", "SitePartyID", "Division",
			"POBoxCity", "POBoxPostalCode", "CustFAX", "TransportZone", "Office", "Floor", "Building", "County",
			"City2" };
	private static final String[] ISRAEL_SKIP_ON_SUMMARY_UPDATE_FIELDS = { "Affiliate", "Company", "CAP", "CMROwner",
			"CustClassCode", "LocalTax1", "LocalTax2", "SearchTerm", "SitePartyID", "StreetAddress2", "Division",
			"POBoxCity", "POBoxPostalCode", "CustFAX", "TransportZone", "Office", "Floor", "Building", "County",
			"City2", "CustomerName2" };

	private static final String[] GREECE_CYPRUS_TURKEY_SKIP_ON_SUMMARY_UPDATE_FIELDS = { "Affiliate", "Company", "CAP",
			"CMROwner", "CustClassCode", "LocalTax1", "LocalTax2", "SearchTerm", "SitePartyID", "Division", "POBoxCity",
			"POBoxPostalCode", "CustFAX", "TransportZone", "Office", "Floor", "Building", "County", "City2",
			"Department" };

	private static final List<String> EMEA_COUNTRY_VAL = Arrays.asList(SystemLocation.UNITED_KINGDOM,
			SystemLocation.IRELAND, SystemLocation.ISRAEL, SystemLocation.TURKEY, SystemLocation.GREECE,
			SystemLocation.CYPRUS, SystemLocation.ITALY);

	private static final List<String> INVALID_CUST_FOR_COUNTRY = Arrays.asList(SystemLocation.UNITED_KINGDOM,
			SystemLocation.IRELAND);

	public static final String EMEA_MASSCHANGE_TEMPLATE_ID = "EMEA";

	private static final List<String> ENABLE_MASSCHANGE_AUTO_TEMPLATE = Arrays.asList(SystemLocation.SPAIN,
			SystemLocation.UNITED_KINGDOM, SystemLocation.IRELAND);
  ...
  "Install At Address" };
	...
	@Override
	public void handleSOFConvertFrom(EntityManager entityManager, FindCMRResultModel source, RequestEntryModel reqEntry,
			FindCMRRecordModel mainRecord, List<FindCMRRecordModel> converted, ImportCMRModel searchModel)
			throws Exception {
		...
		if (CmrConstants.REQ_TYPE_CREATE.equals(reqEntry.getReqType())) {
			...
			if (SystemLocation.UNITED_KINGDOM.equals(record.getCmrIssuedBy())
					|| SystemLocation.IRELAND.equals(record.getCmrIssuedBy())) {
			...
			if (SystemLocation.ISRAEL.equals(record.getCmrIssuedBy())
					|| SystemLocation.SAP_ISRAEL_SOF_ONLY.equals(record.getCmrIssuedBy())) {
				// imported ZS01 will be Country Use A, and imported Country Use
				// A will
				// be ZS01 on the tool. This
				// will be reversed on #doAfterImport
				FindCMRRecordModel mailingSOF = createAddress(entityManager, record.getCmrIssuedBy(),
						CmrConstants.ADDR_TYPE.CTYA.toString(), "Mailing", new HashMap<String, FindCMRRecordModel>());
				if (mailingSOF != null) {
							...
							if (!StringUtils.isBlank(seqNo) && StringUtils.isNumeric(seqNo)) {
								...
								if (CmrConstants.ADDR_TYPE.ZS01.toString().equals(record.getCmrAddrTypeCode())) {
									...
									String adrnr = getaddAddressAdrnr(entityManager,
											SystemConfiguration.getValue("MANDT"), kunnr, addr.getCmrAddrTypeCode(),
											addr.getCmrAddrSeq());
									if (!StringUtils.isBlank(adrnr)) {
										Sadr sadr = getTRAddtlAddr(entityManager, adrnr,
												SystemConfiguration.getValue("MANDT"));
										if (sadr != null) {
											Addr installingAddr = getCurrentInstallingAddress(entityManager,
													reqEntry.getReqId());
              ...
              if (isNewCmrRecordsGR) {
                ...
                break;
					...
					if (SystemLocation.UNITED_KINGDOM.equals(mainRecord.getCmrIssuedBy())
							|| SystemLocation.IRELAND.equals(mainRecord.getCmrIssuedBy())) {
						boolean needToImportMailing = checkIZP01ExistsOnRDCUKI(entityManager,
								mainRecord.getCmrIssuedBy(), mainRecord.getCmrNum());
						// add unmapped addresses
						if (needToImportMailing == false) {
							FindCMRRecordModel record = new FindCMRRecordModel();
							if (LegacyDirectUtil.isCountryLegacyDirectEnabled(entityManager,
									mainRecord.getCmrIssuedBy())) {
								record.setCmrAddrTypeCode(CmrConstants.ADDR_TYPE.ZP01.toString());
								record.setCmrIssuedBy(mainRecord.getCmrIssuedBy());
								setMailingAddressFromLegacy(entityManager, mainRecord.getCmrIssuedBy(), "Mailing",
										record);
							} else {
								record = createAddress(entityManager, mainRecord.getCmrIssuedBy(),
										CmrConstants.ADDR_TYPE.ZP01.toString(), "Mailing",
					...
					for (FindCMRRecordModel record : source.getItems()) {
						...
						if (!SystemLocation.ITALY.equals(record.getCmrIssuedBy())) {
							...
							if (CmrConstants.ADDR_TYPE.ZI01.toString().equals(record.getCmrAddrTypeCode())) {
								if (((!StringUtils.isEmpty(mainRecord.getCmrNum()))
										&& (!StringUtils.isEmpty(record.getCmrCompanyNo())))
										&& (mainRecord.getCmrNum().equalsIgnoreCase(record.getCmrCompanyNo()))) {
									LOG.debug("ZI01:For Italy::CMRNUM==COMPANYNO" + mainRecord.getCmrNum() + "=="
											+ record.getCmrCompanyNo());
							...
							if (CmrConstants.ADDR_TYPE.ZP01.toString().equals(record.getCmrAddrTypeCode())) {
								if (((!StringUtils.isEmpty(mainRecord.getCmrNum()))
										&& (!StringUtils.isEmpty(record.getCmrCompanyNo())))
										&& (mainRecord.getCmrNum().equalsIgnoreCase(record.getCmrCompanyNo()))) {
									LOG.debug("ZP01:For Italy::CMRNUM==COMPANYNO" + mainRecord.getCmrNum() + "=="
											+ record.getCmrCompanyNo());
				...
				if (mainRecord != null) {
					...
					if (sequences != null && !sequences.isEmpty()) {
						...
						if (importShipping) {
							LOG.debug("Shipping Sequences is not empty. Importing " + sequences.size()
									+ " shipping addresses.");
							for (String seq : sequences) {
								record = createAddress(entityManager, cmrCountry,
										CmrConstants.ADDR_TYPE.ZD01.toString(), "Shipping_" + seq + "_", zi01Map);
								if (record != null) {
									if (SystemLocation.GREECE.equals(cmrCountry)
					...
					if (SystemLocation.UNITED_KINGDOM.equals(cmrCountry) || SystemLocation.IRELAND.equals(cmrCountry)) {
						record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.ZP01.toString(),
								"Mailing", zi01Map);
						if (record != null) {
							converted.add(record);
						}
						record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.ZS02.toString(),
								"EplMailing", zi01Map);
						if (record != null) {
							converted.add(record);
						}
						if (!hasInstalling) {
							record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.ZI01.toString(),
									"Installing", zi01Map);
							if (record != null) {
								converted.add(record);
							}
					...
					if (SystemLocation.ISRAEL.equals(cmrCountry)
							|| SystemLocation.SAP_ISRAEL_SOF_ONLY.equals(cmrCountry)) {

						// import all shipping from RDc
						sequences = this.countryCSequences;
						if (sequences != null && !sequences.isEmpty()) {
							LOG.debug("Country Use C Sequences is not empty. Importing " + sequences.size()
									+ " country C addresses.");
							for (String seq : sequences) {
								record = createAddress(entityManager, cmrCountry,
										CmrConstants.ADDR_TYPE.CTYC.toString(), "CtryUseC_" + seq + "_", zi01Map);
								if (record != null) {
									converted.add(record);
								}
							}
							...
							record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.CTYC.toString(),
									"CtryUseC", zi01Map);
							if (record != null) {
								converted.add(record);
							}
						...
						record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.CTYA.toString(),
								"Mailing", zi01Map);
						if (record != null) {
							converted.add(record);
						}
						...
						if (!hasBilling) {
							record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.CTYB.toString(),
									"CtryUseB", zi01Map);
							if (record != null) {
								converted.add(record);
							}
					...
					if (SystemLocation.ITALY.equals(cmrCountry)) {
						record = createAddress(entityManager, cmrCountry, CmrConstants.ADDR_TYPE.ZS01.toString(),
								"Installing", zi01Map);
						if (record != null) {
							converted.add(record);
						}
	...
	private void importUKISequences(List<FindCMRRecordModel> records, String cmrIssuingCntry) {
		...
		for (FindCMRRecordModel record : records) {
			...
			if (!StringUtils.isEmpty(transAddressNo) && StringUtils.isNumeric(transAddressNo)
					&& transAddressNo.length() == 5) {
			...
			if (CmrConstants.ADDR_TYPE.CTYC.toString().equals(countryC.getCmrAddrTypeCode())) {
				if (!allShipping.contains(countryC.getTransAddrNo())) {
					...
					if (unpairedShipping.size() > index) {
						...
						index++;
						LOG.trace("Assigning corrected Sequence no " + countryC.getTransAddrNo() + " to Country C "
								+ countryC.getCmrAddrSeq());
					}
				}
			}
		}
	...
	@Override
	protected void handleSOFAddressImport(EntityManager entityManager, String cmrIssuingCntry,
			FindCMRRecordModel address, String addressKey) {

		String zipCode = this.currentImportValues.get(addressKey + "ZipCode");

		if (SystemLocation.ISRAEL.equals(cmrIssuingCntry)
				|| SystemLocation.SAP_ISRAEL_SOF_ONLY.equals(cmrIssuingCntry)) {
	 ...
	 * @param addressKey
	 */
	private void handleItalySOFAddress(EntityManager entityManager, FindCMRRecordModel address, String cmrIssuingCntry,
			String addressKey) {
	...
	private void handleLocalSOFAddress(EntityManager entityManager, FindCMRRecordModel address, String addressKey) {
		...
		int lineCount = 1 + (!StringUtils.isBlank(line2) ? 1 : 0) + (!StringUtils.isBlank(line3) ? 1 : 0)
				+ (!StringUtils.isBlank(line4) ? 1 : 0);
	 ...
	 * @param addressKey
	 */
	private void handleCrossBorderSOFAddress(EntityManager entityManager, FindCMRRecordModel address,
			String addressKey) {
	 ...
	 * @param addressKey
	 */
	private void handleGreeceCyprusSOFAddress(EntityManager entityManager, FindCMRRecordModel address,
			String cmrIssuingCntry, String addressKey) {
		...
		if (StringUtils.isBlank(address.getCmrCustPhone()) && phone != null
				&& !phone.equals(address.getCmrStreetAddressCont()) && !phone.equals(address.getCmrPOBox())) {
			address.setCmrCustPhone(phone);
		}
	 ...
	 * @param addressKey
	 */
	private void handleTurkeySOFAddress(EntityManager entityManager, FindCMRRecordModel address, String cmrIssuingCntry,
			String addressKey) {
		String name = this.currentImportValues.get(addressKey + "Address1");
	 ...
	 * @throws Exception
	 */
	protected void queryAndAssign(BaseEntity<?> entity, FindCMRResultModel results, FindCMRRecordModel mainRecord)
			throws Exception {
				...
				for (String xmlElement : currentImportValues.keySet()) {
					...
					if (!StringUtils.isBlank(mappedElement) && mappedElement.trim().split("[.]").length == 2) {
						...
						if (entityName.equalsIgnoreCase(entity.getClass().getSimpleName())) {
							...
							try {
								entityField = entityClass
										.getDeclaredField(field.substring(0, 1).toLowerCase() + field.substring(1));
								if (entityField != null) {
	...
	public void setDataValuesOnImport(Admin admin, Data data, FindCMRResultModel results, FindCMRRecordModel mainRecord)
		...
		if (!SystemLocation.ITALY.equalsIgnoreCase(data.getCmrIssuingCntry())) {
			...
			if (SystemLocation.ISRAEL.equals(data.getCmrIssuingCntry())
					&& StringUtils.isEmpty(data.getCollectionCd())) {
			...
			if (SystemLocation.ISRAEL.equals(data.getCmrIssuingCntry())) {
				if (!this.currentImportValues.isEmpty() && !(mainRecord.getCmrCountryLanded().equalsIgnoreCase("IL"))) {
					String country = DropdownListController.getDescription("LandedCountry",
							mainRecord.getCmrCountryLanded(), SystemLocation.ISRAEL);
			...
			if ("U".equals(admin.getReqType())
					&& (SystemLocation.UNITED_KINGDOM.equalsIgnoreCase(data.getCmrIssuingCntry())
			...
			if (((!StringUtils.isEmpty(data.getCompany())) && (!StringUtils.isEmpty(data.getCmrNo())))
				...
				LOG.debug("It is Company CMR (Node1 equal to CMR No):: " + data.getCompany() + " equal to "
						+ data.getCmrNo());
				...
				LOG.debug("It is not Company CMR (Node1 not equal to CMR No): " + data.getCompany() + " not equal to "
						+ data.getCmrNo());
	...
	public void setAddressValuesOnImport(Addr address, Admin admin, FindCMRRecordModel currentRecord, String cmrNo)
		...
		if (currentRecord != null) {
			...
			if ("IL".equals(currentRecord.getCmrCountryLanded())
					&& (SystemLocation.ISRAEL.equals(currentRecord.getCmrIssuedBy())
	...
	public void doBeforeDataSave(EntityManager entityManager, Admin admin, Data data, String cmrIssuingCntry)
		...
		if (cmrIssuingCntry.equals("866") && data.getInacCd() != null) {
			...
			if (count == 1 || data.getInacCd().matches("[a-zA-Z]{1}[a-zA-Z0-9]{3}")) {
				...
				|| SystemLocation.ISRAEL.equals(data.getCmrIssuingCntry())) && data.getInacCd() != null) {
		...
		if ((SystemLocation.UNITED_KINGDOM.equalsIgnoreCase(cmrIssuingCntry)
				|| SystemLocation.IRELAND.equalsIgnoreCase(cmrIssuingCntry))
			...
			if (rdcData != null) {
				...
				// sales rep
				if (StringUtils.isEmpty(data.getRepTeamMemberNo())
						&& !StringUtils.isEmpty(rdcData.getRepTeamMemberNo())) {
		...
		if (SystemLocation.CYPRUS.equals(data.getCmrIssuingCntry())
				|| SystemLocation.GREECE.equals(data.getCmrIssuingCntry())) {
	...
	public void doBeforeAddrSave(EntityManager entityManager, Addr addr, String cmrIssuingCntry) throws Exception {
		...
		case SystemLocation.ISRAEL:
			if ("CTYA".equals(addr.getId().getAddrType())) {
				...
				LOG.debug("Computing Abbreviated Name/Location for Country Use A address of Israel. Request "
						+ addr.getId().getReqId());
				if (data != null && admin.getReqType().equals("C")) {
					...
					if (!"IL".equals(addr.getLandCntry())) {
						...
						String country = DropdownListController.getDescription("LandedCountry", addr.getLandCntry(),
								SystemLocation.ISRAEL);
						if (!StringUtils.isEmpty(country)) {
							data.setAbbrevLocn(country.length() > 12 ? country.substring(0, 12) : country);
						}
					}
		...
		case SystemLocation.GREECE:
			if (data != null && admin.getReqType().equals("C")) {
				if ("ZS01".equals(addr.getId().getAddrType())) {
					LOG.debug("Computing Abbreviated Name/Location for address of Greece. Request "
							+ addr.getId().getReqId());
					data.setAbbrevNm(addr.getCustNm1());
					if (data.getAbbrevNm() != null && data.getAbbrevNm().length() > 22) {
						data.setAbbrevNm(data.getAbbrevNm().substring(0, 22));
		...
		case SystemLocation.CYPRUS:
			if (data != null && admin.getReqType().equals("C")) {
				if ("ZS01".equals(addr.getId().getAddrType())) {
					LOG.debug("Computing Abbreviated Name/Location for address of CYPRUS. Request "
							+ addr.getId().getReqId());
					data.setAbbrevNm(addr.getCustNm1());
					if (data.getAbbrevNm() != null && data.getAbbrevNm().length() > 22) {
						data.setAbbrevNm(data.getAbbrevNm().substring(0, 22));
		...
		case SystemLocation.TURKEY:
			if (data != null && admin.getReqType().equals("C")) {
				if ("ZS01".equals(addr.getId().getAddrType())) {
					LOG.debug("Computing Abbreviated Name/Location for address of TURKEY. Request "
							+ addr.getId().getReqId());
					data.setAbbrevNm(addr.getCustNm1());
					if (data.getAbbrevNm() != null && data.getAbbrevNm().length() > 22) {
						data.setAbbrevNm(data.getAbbrevNm().substring(0, 22));
			...
			if ("ZS01".equals(addr.getId().getAddrType()) || "ZD01".equals(addr.getId().getAddrType())) {
				...
				for (String key : addressDataMap.keySet()) {
						...
						if (!(StringUtils.isEmpty(addressDataMap.get(key)))) {
							for (int i = 0; i < addressDataMap.get(key).length(); i++) {
								...
								if (index >= 0) {
									...
									case '\u00c7':// Ã‡
										field = addressDataMap.get(key).replace(addressDataMap.get(key).charAt(index),
												'C');
					...
					if (!(StringUtils.isEmpty(addressDataMap.get("bldg")))
							&& !(addressDataMap.get("bldg").equals(addr.getBldg()))) {
					...
					if (!(StringUtils.isEmpty(addressDataMap.get("dept")))
							&& !(addressDataMap.get("dept").equals(addr.getDept()))) {
		...
		case SystemLocation.ITALY:
			if ("ZS01".equals(addr.getId().getAddrType())) {
				...
				LOG.debug("Computing Abbreviated Name/Location for Installing address of Italy. Request "
						+ addr.getId().getReqId());
				if (data != null && admin.getReqType().equals("C")) {
					data.setAbbrevNm(addr.getCustNm1());
					if (data.getAbbrevNm() != null && data.getAbbrevNm().length() > 22) {
		...
		case SystemLocation.IRELAND:
			if ("ZI01".equals(addr.getId().getAddrType())) {
				LOG.debug("Computing Abbreviated Name/Location for Installing address of Ireland. Request "
						+ addr.getId().getReqId());
				if (data != null && admin.getReqType().equals("C")) {
			...
			if (!StringUtils.isEmpty(addr.getCustPhone()) && !"ZS01".equals(addr.getId().getAddrType())
					&& !"ZD01".equals(addr.getId().getAddrType())) {
				addr.setCustPhone("");
			}

			if (!Arrays.asList(HRDWRE_MSTR_FLAG_ADDRS).contains(addr.getId().getAddrType())
					&& "Y".equalsIgnoreCase(addr.getHwInstlMstrFlg())) {
		...
		case SystemLocation.UNITED_KINGDOM:
			if ("ZI01".equals(addr.getId().getAddrType())) {
				...
				// location
				LOG.debug("Computing Abbreviated Name/Location for Installing address of UK. Request "
						+ addr.getId().getReqId());
				if (data != null && admin.getReqType().equals("C")) {
					/*
					 * String abbrevNmValue = addr.getCustNm1(); if
					 * (abbrevNmValue != null && abbrevNmValue.length() > 22) {
					 * abbrevNmValue = abbrevNmValue.substring(0, 22); }
					 * data.setAbbrevNm(addr.getCustNm1());
					 */
	...
	public void addSummaryUpdatedFields(RequestSummaryService service, String type, String cmrCountry, Data newData,
			DataRdc oldData, List<UpdatedDataModel> results) {
		...
		if (RequestSummaryService.TYPE_CUSTOMER.equals(type)
				&& !equals(oldData.getEmbargoCd(), newData.getEmbargoCd())) {
	...
	public void doAfterImport(EntityManager entityManager, Admin admin, Data data) {
		...
		if (SystemLocation.IRELAND.equals(data.getCmrIssuingCntry())
				|| SystemLocation.UNITED_KINGDOM.equals(data.getCmrIssuingCntry())
				|| SystemLocation.GREECE.equals(data.getCmrIssuingCntry())
				|| SystemLocation.CYPRUS.equals(data.getCmrIssuingCntry())
		...
		if (SystemLocation.ISRAEL.equals(data.getCmrIssuingCntry()) && data.getAbbrevNm() != null
				&& data.getAbbrevNm().length() > 22) {
			data.setAbbrevNm(data.getAbbrevNm().substring(0, 22));
			entityManager.merge(data);
			entityManager.flush();
		}
	...
	public List<String> getAddressFieldsForUpdateCheck(String cmrIssuingCntry) {
		...
		fields.addAll(
				Arrays.asList("CUST_NM1", "CUST_NM2", "ADDR_TXT", "DEPT", "CITY1", "POST_CD", "LAND_CNTRY", "PO_BOX"));
	...
	public List<String> getDataFieldsForUpdateCheckLegacy(String cmrIssuingCntry) {
		List<String> fields = new ArrayList<>();
		fields.addAll(Arrays.asList("SALES_BO_CD", "REP_TEAM_MEMBER_NO", "SPECIAL_TAX_CD", "VAT", "ISIC_CD",
				"EMBARGO_CD", "COLLECTION_CD", "ABBREV_NM", "SENSITIVE_FLAG", "CLIENT_TIER", "COMPANY", "INAC_TYPE",
				"INAC_CD", "ISU_CD", "SUB_INDUSTRY_CD", "ABBREV_LOCN", "PPSCEID", "MEM_LVL", "BP_REL_TYPE"));
		return fields;
	}
	...
	private void setMailingAddressFromLegacy(EntityManager entityManager, String cmrIssuingCntry, String addressKey,
			FindCMRRecordModel address) {
	...
	protected void handleSOFAddressImportUKI(EntityManager entityManager, String cmrIssuingCntry,
			FindCMRRecordModel address, String addressKey, CmrtAddr cmrtAddr) {

		// UKI order
		/*
		 * line1= name 1 line2=name2/street con't/att+phone/po box/street
		 * line3=street con't/att+phone/po box/street/city
		 * line4=street/city/postal code/cross border country line5=city/postal
		 * code/cross border country line6=postal code/cross border country
		 */
		...
		if (zipLine == 3) {
			...
			address.setCmrPostalCode(
					StringUtils.isBlank(address.getCmrPostalCode()) ? line3 : address.getCmrPostalCode());
		} else if (zipLine == 4) {
			...
			address.setCmrPostalCode(
					StringUtils.isBlank(address.getCmrPostalCode()) ? line4 : address.getCmrPostalCode());
		} else if (zipLine == 5) {
			if (streetLine == 4) {
				if (boxLine == 0) {
			...
			address.setCmrPostalCode(
					StringUtils.isBlank(address.getCmrPostalCode()) ? line5 : address.getCmrPostalCode());
			...
			address.setCmrPostalCode(
					StringUtils.isBlank(address.getCmrPostalCode()) ? line6 : address.getCmrPostalCode());
			if (StringUtils.isBlank(address.getCmrStreetAddress())) {
		...
		if (StringUtils.isEmpty(address.getCmrCountryLanded())
				&& (!StringUtils.isEmpty(line6) || !StringUtils.isEmpty(line5))) {
	...
	public void addSummaryUpdatedFieldsForAddress(RequestSummaryService service, String cmrCountry, String addrTypeDesc,
			String sapNumber, UpdatedAddr addr, List<UpdatedNameAddrModel> results, EntityManager entityManager) {
	...
	public String getaddAddressAdrnr(EntityManager entityManager, String mandt, String kunnr, String ktokd,
			String seq) {
		String adrnr = "";
  ...
  private List<FindCMRRecordModel> handleOldCmrRecordsGR(EntityManager entityManager, FindCMRRecordModel record, String cmrIssueCd, int recCount) throws Exception {
