...
function commentImgFormatter(value, rowIndex) {
  ...
  if (value.indexOf('@') > 0) {
    if (value.indexOf('ibm.com') > 0) {
      return '<img title="' + value + '" src="https://w3-ui-unified-profile-proxy.w3-ui.dal.app.cirrus.ibm.com/up-api-proxy/v3/image/' + value + '" class="cmt-img" onerror="this.onerror=null; this.src=\'' + cmr.CONTEXT_ROOT
  ...
  if (reqId != null && reqId != '' && reqType == 'C' && reqStatus == 'DRA' && _dnbSearchHandler == null) {
    _dnbSearchHandler = dojo.connect(FormManager.getField('custSubGrp'), 'onChange', function(value) {
      ...
      if (FormManager.getActualValue('cmrIssuingCntry') == '760'
          && (FormManager.getActualValue('custSubGrp') == 'RACMR' || FormManager.getActualValue('custSubGrp') == 'BFKSC' || FormManager.getActualValue('custSubGrp') == 'BPWPQ' || FormManager
              .getActualValue('custSubGrp') == 'PRCMR')) {
        cmr.hideNode('dnbRequired');
        cmr.hideNode('dnbRequiredIndc');
      }
    });
