...
function autoSetSBOAndSalesTMNo() {
  var _custSubGrp = FormManager.getActualValue('custSuautoSetSBOAndSalesTMNobGrp');
...
function setIsuCtcForBR(fromAddress, scenario, scenarioChanged) {
  ...
  if (custGrp.includes('LOCAL') && scenarioChanged) {
    if (isucd21.includes(custType)) {
      FormManager.setValue('isuCd', '21');
      FormManager.setValue('clientTier', '');
      FormManager.readOnly('isuCd');
      FormManager.readOnly('clientTier');
    } else if (custType.includes('BUSPR')) {
  ...
  } else if (custGrp.includes('CROSS') && scenarioChanged) {
    FormManager.setValue('isuCd', '21');
    FormManager.setValue('clientTier', '');
    FormManager.readOnly('isuCd');
    FormManager.readOnly('clientTier');
...
function setIsuCTCLockedForReqProcesBR() {
  var _custType = FormManager.getActualValue('custType');
  var _custGrp = FormManager.getActualValue('custGrp');
  if (_custGrp=='CROSS' || (_custGrp=='LOCAL' && (_custType == 'BUSPR' || _custType == 'INTER' || _custType == 'IBMEM' || _custType == 'PRIPE'))) {
    var role = FormManager.getActualValue('userRole').toUpperCase();
...
dojo.addOnLoad(function() {
  ...
  GEOHandler.addAfterConfig(setIsuCTCLockedForReqProcesBR, [ SysLoc.BRAZIL ]);
