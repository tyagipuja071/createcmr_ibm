...
function setCTCIsuByClusterBDLK() {
  ...
  if (custSubGrp == 'AQSTN') {
    ...
    if (_apCustClusterId == '012D999') {
    ...
    else if (_apCustClusterId == '09204') {
      FormManager.limitDropdownValues(FormManager.getField('inacType'), ['I']);
      FormManager.limitDropdownValues(FormManager.getField('clientTier'), ['0']);
      FormManager.setValue('clientTier', '0');
      FormManager.setValue('isuCd', '5K');
      ...
      FormManager.readOnly('inacCd');
      FormManager.readOnly('inacType');
    } else if (_apCustClusterId == '08034' || _apCustClusterId == '12003' || _apCustClusterId == '12004') {
      FormManager.setValue('clientTier', 'Y');
      FormManager.setValue('isuCd', '36');
      FormManager.setValue('inacCd', '');
      FormManager.setValue('inacType', '');
      FormManager.readOnly('inacType');
      FormManager.readOnly('inacCd');
    } else if (_apCustClusterId == '10658') {
      FormManager.setValue('clientTier', 'Q');
      FormManager.setValue('isuCd', '34');
      FormManager.setValue('inacCd', '');
      FormManager.setValue('inacType', '');
    ...
    FormManager.limitDropdownValues(FormManager.getField('clientTier'), ['Y']);
    FormManager.limitDropdownValues(FormManager.getField('apCustClusterId'), ['08034','12003','12004']);
    FormManager.limitDropdownValues(FormManager.getField('isuCd'), ['36']);
    FormManager.setValue('clientTier', 'Y');
    FormManager.setValue('isuCd', '36');
    FormManager.setValue('mrcCd', '3');
...
function setKUKLAvaluesBD() {
  ...
  console.log('>>>> setKUKLAvaluesBD() >>>> set KUKLA values for BD >>>>');
  ...
  if (reqType == 'C') {
    ...
    if (results != null) {
      for (var i = 0; i < results.length; i++) {
        kuklaBD.push(results[i].ret1);
      }
    }

    if (results != null) {
      if (custSubGrp1.has(custSubGrp)) {
        FormManager.setValue('custClass', kuklaBD[0]);
      } else if (custSubGrp2.has(custSubGrp)) {
        if (industryClass1.has(industryClass)) {
          FormManager.setValue('custClass', kuklaBD[1]);
        } else if (industryClass2.has(industryClass)) {
          FormManager.setValue('custClass', kuklaBD[2]);
        } else {
          FormManager.setValue('custClass', kuklaBD[0]);
        }
        if (custSubGrp == 'NRML') {
          if (isicCd == '9500') {
            FormManager.setValue('custClass', kuklaBD[4]);
          }
        }
        ...
        FormManager.setValue('custClass', kuklaBD[5]);
      }
    }
  }
}
...
// GEOHandler.addAfterConfig(setCtcOnIsuCdChangeISA, GEOHandler.ISA);
  ...
  GEOHandler.registerValidator(addARCodeValidator, [ SysLoc.BANGLADESH ], null, true); 
