...
function setSBOFromDBMapping(){
  ...
  var sboList = [];
	 var qParams = {
		...
		var results = cmr.query('GET_SBO_CEE', qParams);
...
function setCovValues2024CEE(){
  ...
  var cntry = FormManager.getActualValue('cmrIssuingCntry');
    ...
    case SysLoc.AZERBAIJAN:
    case SysLoc.ARMENIA:
    case SysLoc.GEORGIA:
    case SysLoc.KAZAKHSTAN:
    case SysLoc.KYRGYZSTAN:
    case SysLoc.TURKMENISTAN:
...
function subIndustryLogicCEE(){
	...
	if('34Q' == isuCTC){
		switch (cntry){
			case SysLoc.CZECH_REPUBLIC :
			if(['A','Q','U'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW1');
			}else if(['B','C'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW3');
			}else if(['D','R','T','W'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW2');
			}else if(['F','N','S'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW5');
			}else if(['J','L','M','P','V'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW6');
			}else if(['E','G','H','X','Y'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','TW4');
			}			
			break;
			...
			if(['A','K'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P12');
			}else if(['B','C'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P07');
			}else if(['D','J','L','M','P','R','T','V'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P20');
			}else if(['E','G','Y'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P08');
			}else if(['F'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P05');
			}else if(['X','H'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P10');
			}else if(['N'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P09');
			}	else if(['S'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P06');
			}	else if(['U'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P24');
			}else if(['W'].includes(subInd)){
				FormManager.setValue('salesBusOffCd','P23');
			}				
			break;
		}
	}
...
dojo.addOnLoad(function () {
 GEOHandler.CEE = ['358', '359', '363', '603', '607', '626', '644', '651', '668', '693', '694', '695', '699', '704', '705', '707', '707', '708', '713', '740', '741', '787', '820', '821',
  ...
  GEOHandler.addAddrFunction(updateMainCustomerNames, GEOHandler.CEE);
  GEOHandler.setRevertIsicBehavior(false);

  GEOHandler.addAfterConfig(afterConfigForCEMEA, GEOHandler.CEE);
  GEOHandler.addAfterConfig(addHandlersForCEMEA, GEOHandler.CEE);
  GEOHandler.addAfterConfig(addVatExemptHandler, GEOHandler.CEE);
  GEOHandler.addAfterConfig(addCISHandler, [SysLoc.RUSSIA]);
  GEOHandler.addAfterConfig(setAbbrvNmLoc, GEOHandler.CEE);
  GEOHandler.addAfterConfig(lockAbbrv, GEOHandler.CEE);
   
  GEOHandler.addAfterConfig(setEnterpriseValues, GEOHandler.CEE);
  GEOHandler.addAfterConfig(setVatRequired, GEOHandler.CEE);
  GEOHandler.addAfterConfig(setPreferredLang, GEOHandler.CEE);
  GEOHandler.addAfterTemplateLoad(setVatRequired, GEOHandler.CEE);
  
  GEOHandler.addAfterConfig(setCommercialFinanced, GEOHandler.CEE);
  GEOHandler.addAfterTemplateLoad(setCommercialFinanced, GEOHandler.CEE);
  GEOHandler.addAfterConfig(setTelecoverageRep, GEOHandler.CEE);
  GEOHandler.addAfterTemplateLoad(setTelecoverageRep, GEOHandler.CEE);
  GEOHandler.addAfterConfig(lockLandCntry, GEOHandler.CEE);
  GEOHandler.addAfterTemplateLoad(lockLandCntry, GEOHandler.CEE);
  GEOHandler.addAfterConfig(setAbbrvNmLocMandatoryProcessor, GEOHandler.CEE);
  GEOHandler.addAfterConfig(setVatValidator, GEOHandler.CEE);
  GEOHandler.addAfterTemplateLoad(setVatValidator, GEOHandler.CEE);
  ...
  GEOHandler.addAfterTemplateLoad(afterConfigForCEMEA, GEOHandler.CEE);
  ...
  GEOHandler.addAfterConfig(resetVatExemptOnchange, GEOHandler.CEE);
  GEOHandler.addAddrFunction(changeAbbrevNmLocn, GEOHandler.CEE);
  GEOHandler.addAfterConfig(validateAbbrevNmLocn, GEOHandler.CEE);
  GEOHandler.addAddrFunction(addLatinCharValidator, GEOHandler.CEE);

  GEOHandler.addAfterTemplateLoad(setPreferredLang, GEOHandler.CEE);

  GEOHandler.registerValidator(addAddressFieldValidators, GEOHandler.CEE, null, true);
  ...
  GEOHandler.registerValidator(cemeaCustomVATValidator('', 'MAIN_CUST_TAB', 'frmCMR', 'ZP01'), GEOHandler.CEE, null, true);
...
//  GEOHandler.registerValidator(addCEMEAChecklistValidator, GEOHandler.CEE);
  ...
  GEOHandler.addToggleAddrTypeFunction(toggleLocalCountryName, GEOHandler.CEE);
  GEOHandler.addAddrFunction(toggleLocalCountryNameOnOpen, GEOHandler.CEE);
