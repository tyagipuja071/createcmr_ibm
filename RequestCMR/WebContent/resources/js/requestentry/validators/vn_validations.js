...
function setDefaultARForVN() {
  console.log('>>>> setDefaultARForVN >>>>');
  if (FormManager.getActualValue('viewOnlyPage') === 'true') {
    return;
  }
...
function updateMRCAseanAnzIsa() {
  ...
  if (_isuCd != null && _isuCd.length > 1) {
    ...
    if (_exsitFlag == 0) {
      if (cntry == '852' && (custSubGrp == 'ECSYS' || custSubGrp == 'CROSS' || custSubGrp == 'ASLOM' || custSubGrp == 'BUSPR')) {
        FormManager.setValue('mrcCd', '3');
      } 
    }
...
function freeTxtFieldShowHide(buttonNo) {
  ...
  var element = document.getElementById('checklist_txt_field_' + 1);
  var textFieldElement = document.getElementsByName('freeTxtField' + 1)[0];
  if (shouldDisplay) {
...
function checkChecklistButtons() {
    if (document.getElementById('dijit_form_RadioButton_' + 2).checked) {
      document.getElementById('checklist_txt_field_' + 1).style.display = 'block';
    }
}
...
function updateChecklist() {
	...
	var result = cmr.query('ADDR.GET.LANDCNTRY.BY_REQID', qParams);
	landCntry = result.ret1;
  var subHeaders = dojo.query('th.subheader'); 
	var ques = dojo.query('table.checklist-questions');

	if (landCntry != 'KH') {
		ques[4].hidden = true;
		ques[5].hidden = true;
		for(i = 4;i<=8;i++){
			subHeaders[i].hidden = true;
		}
   dojo.byId('dijit_form_TextBox_3').hidden = true; 
   dojo.byId('dijit_form_TextBox_4').hidden = true; 
	} else {
		ques[4].hidden = false;
		ques[5].hidden = false;
		for(i = 5;i<=8;i++){
			subHeaders[i].hidden = false;
		}
	 dojo.byId('dijit_form_TextBox_3').hidden = false;
   dojo.byId('dijit_form_TextBox_4').hidden = false; 
	}
...
function setKUKLAvaluesVN() {
  ...
  console.log('>>>> setKUKLAvaluesVN() >>>> set KUKLA values for VN >>>>');
  ...
  if (reqType == 'C') {
    ...
    if (results != null) {
      for (var i = 0; i < results.length; i++) {
        kuklaVN.push(results[i].ret1);
      }
    }

    if (results != null) {
      if (custSubGrp1.has(custSubGrp)) {
        FormManager.setValue('custClass', kuklaVN[0]);
      } else if (custSubGrp2.has(custSubGrp)) {
        if (industryClass1.has(industryClass)) {
          FormManager.setValue('custClass', kuklaVN[1]);
        } else if (industryClass2.has(industryClass)) {
          FormManager.setValue('custClass', kuklaVN[2]);
        } else {
          FormManager.setValue('custClass', kuklaVN[0]);
        }
      } else if (custSubGrp3.has(custSubGrp)) {
        FormManager.setValue('custClass', kuklaVN[3]);
      } else if (custSubGrp4.has(custSubGrp)) {
        FormManager.setValue('custClass', kuklaVN[4]);
      } else if (custSubGrp5.has(custSubGrp)) {
        FormManager.setValue('custClass', kuklaVN[5]);
      }
    }
  }
}
...
dojo.addOnLoad(function () {
  ...
  GEOHandler.addAfterConfig(setChecklistStatus, [SysLoc.VIETNAM]);
  GEOHandler.registerValidator(addChecklistValidator, [SysLoc.VIETNAM]);
