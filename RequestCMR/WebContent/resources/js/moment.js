        ...
        function() {
          ...
          function valid__isValid(m) {
            if (m._isValid == null) {
              ...
              if (m._strict) {
                m._isValid = m._isValid && flags.charsLeftOver === 0 && flags.unusedTokens.length === 0
          ...
          function loadLocale(name) {
            var oldLocale = null;
            // TODO: Find a better way to register and load all the locales in
            // Node
          ...
          var matchOffset = /Z|[+-]\d\d:?\d\d/gi; // +00:00 -00:00 +0000 -0000
                                                  // or Z
          ...
          var isoDates = [ [ 'YYYYYY-MM-DD', /[+-]\d{6}-\d{2}-\d{2}/ ], [ 'YYYY-MM-DD', /\d{4}-\d{2}-\d{2}/ ],
              [ 'GGGG-[W]WW-E', /\d{4}-W\d{2}-\d/ ], [ 'GGGG-[W]WW', /\d{4}-W\d{2}/ ], [ 'YYYY-DDD', /\d{4}-\d{3}/ ] ];

          // iso time formats and regexes
          var isoTimes = [ [ 'HH:mm:ss.SSSS', /(T| )\d\d:\d\d:\d\d\.\d+/ ], [ 'HH:mm:ss', /(T| )\d\d:\d\d:\d\d/ ],
          ...
          function dayOfYearFromWeeks(year, week, weekday, firstDayOfWeekOfYear, firstDayOfWeek) {
            var week1Jan = 6 + firstDayOfWeek - firstDayOfWeekOfYear, janX = createUTCDate(year, 0, 1 + week1Jan), d = janX
                .getUTCDay(), dayOfYear;
            if (d < firstDayOfWeek) {
              d += 7;
            }
          ...
          function configFromStringAndArray(config) {
            ...
            for (i = 0; i < config._f.length; i++) {
              ...
              // if there is any input that was not parsed add a penalty for
              // that format
          ...
          function moment_calendar__calendar(time, formats) {
            ...
            // Getting start-of-today depends on whether we're local/utc/offset
            // or not.
            var now = time || local__createLocal(), sod = cloneWithOffset(now, this).startOf('day'), diff = this.diff(
                sod, 'days', true), format = diff < -6 ? 'sameElse' : diff < -1 ? 'lastWeek' : diff < 0 ? 'lastDay'
          ...
          // specified which hour he wants. So trying to maintain the same hour
          // (in
          ...
          function relative__relativeTime(number, withoutSuffix, string, isFuture) {
            ...
            return (typeof output === 'function') ? output(number, withoutSuffix, string, isFuture) : output.replace(
                /%d/i, number);
          }
          ...
          locale_locales__getSetGlobalLocale('en', {
            ...
            ordinal : function(number) {
              var b = number % 10, output = (toInt(number % 100 / 10) === 1) ? 'th' : (b === 1) ? 'st'
                  : (b === 2) ? 'nd' : (b === 3) ? 'rd' : 'th';
          ...
          function bubble() {
            ...
            // if we have a mix of positive and negative values, bubble down
            // first
          ...
          function duration_humanize__relativeTime(posNegDuration, withoutSuffix, locale) {
            ...
            var a = seconds < thresholds.s && [ 's', seconds ] || minutes === 1 && [ 'm' ] || minutes < thresholds.m
                && [ 'mm', minutes ] || hours === 1 && [ 'h' ] || hours < thresholds.h && [ 'hh', hours ] || days === 1
                && [ 'd' ] || days < thresholds.d && [ 'dd', days ] || months === 1 && [ 'M' ] || months < thresholds.M
