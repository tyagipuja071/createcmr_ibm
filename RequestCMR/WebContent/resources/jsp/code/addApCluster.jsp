...
<%
  ...
  ApClusterModel apClusterModel = (ApClusterModel) request.getAttribute("apClusterModel");
  boolean newEntry = false;
  String countryDesc = "";
  if (apClusterModel.getState() == BaseModel.STATE_NEW) {
    newEntry = true;
  }
...
<div ng-app="APCLUSTERApp" ng-controller="APCLUSTERController" ng-cloak>
	<cmr:boxContent>
		<cmr:tabs />
		...
		<cmr:section>
			<cmr:row topPad="8">
				<cmr:column span="6">
					<h2><%=newEntry ? "Add AP Cluster Mappings" : "Maintain AP Cluster Mappings"%>
						<%
			...
			<cmr:row>
				<table id="recipientsTable" cellspacing="0" cellpadding="0"
					...
					<thead>
						<tr>
						    <th scope="col" width="2%">Default</th>
							<th scope="col" width="8%">AP Cluster ID</th>
							<th scope="col" width="*">AP Cluster Description</th>
							<th scope="col" width="17%">ISU Codes</th>
							<th scope="col" width="17%">Client Tier Code</th>
							<th scope="col" width="8%">Actions</th>
						</tr>
					</thead>
					<tbody>
						<tr ng-show="apClusterItems.length == 0">
							<td colspan="6">No data to show</td>
							<td><img class="add" title="Add Mapping"
								ng-click="addNew(0)" src="${resourcesPath}/images/add.png">
						</tr>
						<tr ng-repeat="apCluster in apClusterItems track by $index"
							class="{{$index%2 == 1 ? 'ibm-alt-row' : ''}}">
							<td>
							
							<input id="apCluster.apCustClusterId" name="apCluster.apCustClusterId" ng-model="apCluster.defaultIndc" type="checkbox"  ng-true-value="'Y'" ng-false-value="''""
							ng-disabled="!apCluster.edit"  ng-click= "updateSelection($index, apClusterItems,apCluster.apCustClusterId)">
							</td>
							<td><input ng-model="apCluster.apCustClusterId" maxlength="6"
								ng-readonly="!apCluster.edit"
								ng-class="{'lov-ro' : !apCluster.edit, 'lov-del' : apCluster.status == 'D'}"
								style="width: 95%"></td>
							<td><input ng-model="apCluster.clusterDesc" maxlength="50"
								ng-readonly="!apCluster.edit"
								ng-class="{'lov-ro' : !apCluster.edit, 'lov-del' : apCluster.status == 'D'}"
								style="width: 95%"></td>
							<td><input ng-model="apCluster.isuCode" readonly="true"
								ng-class="{'lov-ro' : !apCluster.edit, 'lov-del' : apCluster.status == 'D'}"
								style="width: 75%"> &nbsp;<img class="up"
								title="Edit ISU" ng-click="!apCluster.edit||editISUCTC($index,'isu')"
								src="${resourcesPath}/images/edit.png"></td>
							<td><input ng-model="apCluster.clientTierCd" readonly="true"
								ng-class="{'lov-ro' : !apCluster.edit, 'lov-del' : apCluster.status == 'D'}"
								style="width: 75%"> &nbsp;<img class="up"
								title="Edit Client Tier"
								ng-click="!apCluster.edit||editISUCTC($index,'ctc')"
								src="${resourcesPath}/images/edit.png"></td>
							<td><img class="add" title="Add Mapping"
								ng-click="addNew($index+1)" src="${resourcesPath}/images/add.png">
								<img ng-show="!apCluster.edit && apCluster.status!='D'" class="add" title="Edit Mapping"
	...
	<cmr:section alwaysShown="true">
		<cmr:buttonsRow>
			...
			<input style="margin-left: 10px" type="button"
				...
				value="Delete All Mappings" ng-click="deleteApCluster()">
			<input style="margin-left: 10px" type="button"
...
<script src="${resourcesPath}/js/system/apCustClusterMap.js?${cmrv}"></script>
