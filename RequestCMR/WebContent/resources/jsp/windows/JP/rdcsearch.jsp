...
<%
  ...
  boolean shouldShowField = false;
  if(scenario == null) {
  	scenario = "";
  }
  
  if(scenario.equals("BFKSC") || scenario.equals("RACMR")) {
...
console.log("shouldShowField: '<%=shouldShowField%>'");
...
<cmr:window>
  <cmr:form method="POST" action="<%=actionUrl%>" name="frmCMR" class="ibm-column-form ibm-styled-form" modelAttribute="crit">
    ...
    <%if (shouldShowField){%>
     ...
     <%} %>
      <cmr:column span="1" width="150">
        <label for="accountCredtitToCustNo">Credit CMR/IBM Related CMR:</label>
      </cmr:column>
      <cmr:column span="2" width="350">
  ...
  <cmr:windowClose>
    ...
    <cmr:button label="Clear Search Criteria" onClick="window.location = 'rdcsearch?clear=Y'" highlight="false" pad="true"/>
  </cmr:windowClose>
</cmr:window>
