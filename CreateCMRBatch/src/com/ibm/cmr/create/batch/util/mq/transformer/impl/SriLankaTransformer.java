...
public class SriLankaTransformer extends ISATransformer {
  private static final Logger LOG = Logger.getLogger(SriLankaTransformer.class);

  public SriLankaTransformer() throws Exception {
    super(SystemLocation.SRI_LANKA);

  }

  @Override
  protected String getDoubleCreateCountry(EntityManager entityManager, MQMessageHandler handler) {
    if (handler.cmrData.getCmrIssuingCntry().equals(SystemLocation.SRI_LANKA) && "C".equals(handler.adminData.getReqType()))
  ...
  public void formatDataLines(MQMessageHandler handler) {
    ...
    handler.messageHash.put("CntryNo", SystemLocation.SRI_LANKA);

    /*
     * if (!StringUtils.isEmpty(handler.mqIntfReqQueue.getCorrelationId()) &&
     * handler.cmrData.getCmrIssuingCntry().equals(SystemLocation.SRI_LANKA)) {
