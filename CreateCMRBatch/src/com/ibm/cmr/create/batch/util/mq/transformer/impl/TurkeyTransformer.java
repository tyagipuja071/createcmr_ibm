...
public class TurkeyTransformer extends EMEATransformer {
	...
	public boolean isCrossBorderForMass(MassUpdtAddr addr, CmrtAddr legacyAddr) {
		...
		if (!StringUtils.isEmpty(addr.getLandCntry())
				&& !SpainTransformer.DEFAULT_LANDED_COUNTRY.equals(addr.getLandCntry())) {
			isCrossBorder = true;
	...
	public void transformLegacyAddressData(EntityManager entityManager, MQMessageHandler dummyHandler,
		...
		if ("N".equals(currAddr.getImportInd()) && MQMsgConstants.ADDR_ZP01.equals(currAddr.getId().getAddrType())) {
			...
			legacyAddr.getId().setAddrNo(StringUtils.isEmpty(currAddr.getPrefSeqNo()) ? legacyAddr.getId().getAddrNo()
					: currAddr.getPrefSeqNo());

			legacyAddr.setIsAddrUseMailing("Y");
			legacyAddr.setIsAddrUseBilling("Y");
	...
	@Override
	public void generateCMRNoByLegacy(EntityManager entityManager, GenerateCMRNoRequest generateCMRNoObj,
	...
	@Override
	public void transformLegacyAddressDataMassUpdate(EntityManager entityManager, CmrtAddr legacyAddr,
			MassUpdtAddr addr, String cntry, CmrtCust cust, Data data, LegacyDirectObjectContainer legacyObjects) {
		...
		if (!StringUtils.isBlank(addr.getCustNm1())) {
			...
			if (legacyFiscalAddr != null) {
				String prefix = !StringUtils.isEmpty(legacyFiscalAddr.getAddrLine1())
						? legacyFiscalAddr.getAddrLine1().substring(0, 1) : "";
		...
		if (!StringUtils.isBlank(addr.getPostCd())) {
			...
			if (CmrConstants.ADDR_TYPE.ZS01.toString().equals(addr.getId().getAddrType())
					&& isCrossBorderForMass(addr, legacyAddr)) {
	...
	@Override
	public void formatMassUpdateAddressLines(EntityManager entityManager, CmrtAddr legacyAddr,
			MassUpdtAddr massUpdtAddr, boolean isFAddr) {
		...
		if (isFAddr) {
			...
			for (String part : parts) {
				if (!StringUtils.isEmpty(part)
						&& (StringUtils.isNumeric(part) || (part.matches(".*\\d{1}.*") && part.contains("-")))) {
					line3 = part;
		...
		String[] lines = new String[] { (line1 != null ? line1.trim() : ""), (line2 != null ? line2.trim() : ""),
				(line3 != null ? line3.trim() : ""), (line4 != null ? line4.trim() : ""),
				(line5 != null ? line5.trim() : "") };
		int lineNo = 1;
	...
	@Override
	public void transformLegacyCustomerData(EntityManager entityManager, MQMessageHandler dummyHandler,
			CmrtCust legacyCust, CMRRequestContainer cmrObjects) {
		...
		if (CmrConstants.REQ_TYPE_CREATE.equals(admin.getReqType())) {
			legacyCust.setLangCd(StringUtils.isEmpty(legacyCust.getLangCd())
					? dummyHandler.messageHash.get("CustomerLanguage") : legacyCust.getLangCd());
      ...
      if (!StringUtils.isBlank(data.getSalesBusOffCd())) {
        ...
        if (!StringUtils.isBlank(isr)) {
          ...
          cmrObjects.getData().setRepTeamMemberNo("");
			...
			// Mukesh : Story 1597678: Support temporary reactivation requests
			// due to
			// Embargo Code handling
			if (admin.getReqReason() != null && !StringUtils.isBlank(admin.getReqReason())
					&& CMR_REQUEST_REASON_TEMP_REACT_EMBARGO.equals(admin.getReqReason())
					&& admin.getReqStatus() != null && admin.getReqStatus().equals(CMR_REQUEST_STATUS_CPR)
					&& (rdcEmbargoCd != null && !StringUtils.isBlank(rdcEmbargoCd)) && "E".equals(rdcEmbargoCd)
					&& (dataEmbargoCd == null || StringUtils.isBlank(dataEmbargoCd))) {
				legacyCust.setEmbargoCd("");
			...
			if (admin.getReqReason() != null && !StringUtils.isBlank(admin.getReqReason())
					&& CMR_REQUEST_REASON_TEMP_REACT_EMBARGO.equals(admin.getReqReason())
					&& admin.getReqStatus() != null && admin.getReqStatus().equals(CMR_REQUEST_STATUS_PCR)
					&& (rdcEmbargoCd != null && !StringUtils.isBlank(rdcEmbargoCd)) && "E".equals(rdcEmbargoCd)
					&& (dataEmbargoCd == null || StringUtils.isBlank(dataEmbargoCd))) {
		...
		if (!StringUtils.isEmpty(data.getCollectionCd())) {
			legacyCust.setDistrictCd(data.getCollectionCd());
		}
	...
	@Override
	public void transformLegacyCustomerDataMassUpdate(EntityManager entityManager, CmrtCust cust,
			CMRRequestContainer cmrObjects, MassUpdtData muData) { // default
	...
	protected void handleDataDefaults(MQMessageHandler handler, Map<String, String> messageHash, Data cmrData,
			boolean crossBorder, Addr addrData) {
		messageHash.put("SourceCode", "EF0");
		messageHash.put("CEdivision", "2");
	...
	public void transformLegacyCustomerExtData(EntityManager entityManager, MQMessageHandler dummyHandler,
		...
		for (Addr addr : cmrObjects.getAddresses()) {
			...
			if (MQMsgConstants.ADDR_ZP01.equals(addr.getId().getAddrType())) {
				...
				legacyCustExt
						.setItBillingCustomerNo(!StringUtils.isBlank(addr.getParCmrNo()) ? addr.getParCmrNo() : "");
