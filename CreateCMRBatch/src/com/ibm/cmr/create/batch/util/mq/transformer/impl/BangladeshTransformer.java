...
public class BangladeshTransformer extends ISATransformer {

  private static final Logger LOG = Logger.getLogger(BangladeshTransformer.class);

  public BangladeshTransformer() throws Exception {
    super(SystemLocation.BANGLADESH);

  }

  @Override
  protected String getDoubleCreateCountry(EntityManager entityManager, MQMessageHandler handler) {
    if (handler.cmrData.getCmrIssuingCntry().equals(SystemLocation.BANGLADESH) && "C".equals(handler.adminData.getReqType()))
      return SystemLocation.SINGAPORE; // All CMR types
    return "";
  }
  ...
  public void formatDataLines(MQMessageHandler handler) {
     ...
     * handler.cmrData.getCmrIssuingCntry().equals(SystemLocation.BANGLADESH)) {
    ...
    if (MQMsgConstants.REQ_TYPE_CREATE.equals(handler.mqIntfReqQueue.getReqType())
      ...
      /*
       * String sql = ExternalizedQuery.getSql("GET.CHECK_REPROCESS");
       * PreparedQuery query = new PreparedQuery(handler.getEntityManager(),
       * sql); query.setParameter("REQ_ID", handler.mqIntfReqQueue.getReqId());
       * query.setParameter("COUNTRY", handler.cmrData.getCmrIssuingCntry());
       * query.setParameter("CURRENT_ID",
       * handler.mqIntfReqQueue.getId().getQueryReqId()); if (query.exists()) {
       * // handle reprocessing of double create parent request
       * handler.messageHash.put("TransCode", "M"); } else { // first attempt
       * for double create request handler.messageHash.put("TransCode", "N"); }
       */
     ...
     * query.setParameter("REQ_ID", handler.mqIntfReqQueue.getReqId());
     * query.setParameter("COUNTRY",
     * handler.mqIntfReqQueue.getCmrIssuingCntry());
     * query.setParameter("CURRENT_ID",
     * handler.mqIntfReqQueue.getId().getQueryReqId()); if (query.exists() &&
     * "".equals(cmrPrefix)) { // handle reprocessing of double create parent
     * request handler.messageHash.put("TransCode", "M"); } else { // first
     * attempt for double create request handler.messageHash.put("TransCode",
     * "N"); } }
     */
  }
  ...
  public void formatAddressLines(MQMessageHandler handler) {
     ...
     * LandedCountryMap.getCountryName(addrData.getLandCntry()) + ">"; if
     * ("LA".equalsIgnoreCase(addrData.getLandCntry())) line66 = "<Laos>"; if
     * (addrData.getPostCd() != null) line66 += " " + addrData.getPostCd();
     ...
     * 
     * String addr5 = handler.messageHash.get("AddrLine5"); String addr6 =
