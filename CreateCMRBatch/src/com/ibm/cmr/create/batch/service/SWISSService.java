...
public class SWISSService extends BaseBatchService {
  ...
  private List<Admin> getPendingRecords(EntityManager entityManager) {
    String sql = ExternalizedQuery.getSql("SWISS.GET_PENDING.RDC");
    ...
    if (isTempReactivate) {
      ...
      if ((admin.getRdcProcessingTs() == null || isTimeStampEquals(admin.getRdcProcessingTs())) && "N".equals(admin.getProcessedFlag())) {
        ...
        try {
          ...
          isDataUpdated = SWISSHandler.isDataUpdated(data, dataRdc, data.getCmrIssuingCntry());
        ...
        if (addresses != null && addresses.size() > 0) {
          for (Addr addr : addresses) {
            ...
            if (SystemLocation.SWITZERLAND.equals(data.getCmrIssuingCntry())) {
              SWISSHandler swHandler = new SWISSHandler();
  ...
  protected void processReactivateDeleteRequest(EntityManager entityManager, Admin admin, Data data) throws Exception {
    ...
    if (input.getInputReqType() != null && (input.getInputReqType().equalsIgnoreCase("D") || input.getInputReqType().equalsIgnoreCase("R"))) {
      request = prepareReactivateDelRequestSwiss(entityManager, admin, data, input);
    }
   ...
   * @throws Exception
   */
  public MassProcessRequest prepareReactivateDelRequestSwiss(EntityManager entityManager, Admin admin, Data data, MassUpdateServiceInput input)
      throws JsonGenerationException, JsonMappingException, IOException, Exception {
  ...
  private ProcessResponse sendAddrForProcessing(Addr addr, ProcessRequest request, List<ProcessResponse> responses, boolean isIndexNotUpdated,
    ...
    // call the service
    if (isTempReactivate) {
      LOG.info("Sending request to Swiss Service for TREC(Temporary Reactivate) [Request ID: " + request.getReqId() + " CMR No: " + request.getCmrNo()
          + " Type: " + request.getReqType() + " SAP No: " + request.getSapNo() + "]");
    } else {
      LOG.info("Sending request to SwissService [Request ID: " + request.getReqId() + " CMR No: " + request.getCmrNo() + " Type: "
          + request.getReqType() + " SAP No: " + request.getSapNo() + "]");
    }
    LOG.trace("Request JSON:");
    if (LOG.isTraceEnabled()) {
    ...
    if (isTempReactivate) {
      ...
      LOG.info("Response received from SwissProcessService [Request ID: " + response.getReqId() + " CMR No: " + response.getCmrNo() + " KUNNR: "
          + addr.getSapNo() + " Status: " + response.getStatus() + " Message: " + (response.getMessage() != null ? response.getMessage() : "-")
          + "]");
    }
    if (response != null && response.getRecords() != null && response.getRecords().size() > 0) {
