...
public class TransConnService extends BaseBatchService {
  ...
  protected Boolean executeBatch(EntityManager entityManager) throws Exception {
    try {
      ...
      LOG.info("Processing LA Reprocess RDC records... (Transconn Removed)");
  ...
  protected List<Long> gatherTransConnRecords(EntityManager entityManager) {
    ...
    LOG.debug("Size of Notify List : " + 0);
    LOG.info("gatherTransConnRecords -- removed");
    List<Long> queue = new ArrayList<>();
  ...
  /**
   * Prepares a list of request IDs to reprocess *
   * 
   * @param entityManager
   * @return
   */
  ...
  protected void processUpdateRequest(EntityManager entityManager, ProcessRequest request, Admin admin, Data data) throws Exception {
    ...
    try {
      ...
      // successful directly
      // if (!("760".equals(data.getCmrIssuingCntry()) && addresses != null &&
      // addresses.size() == 1
      ...
      for (Addr addr : addresses) {
        ...
        if (continueUpdate) {
          ...
          if (StringUtils.isBlank(data.getCmrNo()) && "ZC01".equals(addr.getId().getAddrType())) {
            request.setCmrNo(addr.getParCmrNo());
          }
