...
public class TransConnService extends BaseBatchService {
	...
	protected static final List<String> SINGLE_REQUEST_TYPES = Arrays.asList(CmrConstants.REQ_TYPE_CREATE,
			CmrConstants.REQ_TYPE_UPDATE);
	protected static final List<String> MASS_REQUEST_TYPES = Arrays.asList(CmrConstants.REQ_TYPE_MASS_UPDATE,
			CmrConstants.REQ_TYPE_REACTIVATE, CmrConstants.REQ_TYPE_DELETE);
	...
	/**
	 * Retrieves {@link Admin} records with RDC_PROCESSING_STATUS = 'A'
	 * (Aborted)
	 * 
	 * @param entityManager
	...
	public void monitorAbortedRecords(EntityManager entityManager, List<Long> abortedRecords)
		...
		for (Long id : abortedRecords) {
			try {
				...
				LOG.info("Processing Aborted Record " + admin.getId().getReqId() + " [Request ID: "
				...
				if (SINGLE_REQUEST_TYPES.contains(admin.getReqType())
					...
					LOG.warn("Request ID " + admin.getId().getReqId()
							+ " cannot be processed. Improper Type or not completed.");
				}
	...
	public void monitorMQInterfaceRequests(EntityManager entityManager, List<Long> mqIntfList)
		...
		for (Long id : mqIntfList) {
			...
			isError = mqIntfReq.getReqStatus() != null
					&& mqIntfReq.getReqStatus().contains(MQMsgConstants.REQ_STATUS_SER);
			try {
				LOG.info("Processing MQ Intf Req " + mqIntfReq.getId().getQueryReqId() + " [Request ID: "
						+ mqIntfReq.getReqId() + "]");
				...
				requestQuery.setParameter("REQ_STATUS", isError ? CmrConstants.REQUEST_STATUS.PPN.toString()
				...
				if (wfHist != null) {
					...
					wfHist.setCmt(isError ? MQMsgConstants.WH_COMMENT_REJECT
							: MQMsgConstants.WH_HIST_CMT_COM + data.getCmrNo());
					...
					wfHist.setReqStatus(isError ? CmrConstants.REQUEST_STATUS.PPN.toString()
							: CmrConstants.REQUEST_STATUS.COM.toString());
					wfHist.setRejReason("");
				}
				...
				LOG.error("Error in processing TransConn Record " + mqIntfReq.getId().getQueryReqId()
						+ " for Request ID " + mqIntfReq.getReqId() + " [" + e.getMessage() + "]", e);
			}
		}
	}
	...
	protected void monitorLegacyPending(EntityManager entityManager, List<Long> pvcRecords) {
		...
		for (Long id : pvcRecords) {
			try {
				...
				LOG.info("Processing PVC Record " + admin.getId().getReqId() + " [Request ID: "
						+ admin.getId().getReqId() + "]");
				...
				for (CompanyRecordModel record : records) {
					...
					if (queryReservedCMR.exists() && (records.indexOf(record) == (records.size() - 1))) {
						LOG.error(
								"All CMR numbers in the Pool are existing in reserved. Cannot proceed with automatic CMR number assignment");
						return;
					...
					// set to aborted so the details can be sent to processing
					// service on
					// next run
					...
					for (Kna1 kna1 : kna1List) {
						...
						if ("ZS01".equals(kna1.getKtokd())
								|| ("ZP01".equals(kna1.getKtokd()) && !"PG".equals(kna1.getAufsd()))) {
					...
					RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin,
							"Completed using Pool CMR assignment", "Pool Assignment", null, null, true);
					RequestUtils.createCommentLogFromBatch(entityManager, BATCH_USER_ID, admin.getId().getReqId(),
							"Completed using Pool CMR assignment");

					// Create a new Update request using the CMR_NO from Pool
					// and
					...
					// get a request id, get the request and update data from
					// the original
					// request
					...
					PreparedQuery addrQuery = new PreparedQuery(entityManager,
							ExternalizedQuery.getSql("BATCH.GET_ADDR_ENTITY_CREATE_REQ"));
					...
					PreparedQuery zi01AddrQuery = new PreparedQuery(entityManager,
							ExternalizedQuery.getSql("BATCH.GET_ADDR_ENTITY_CREATE_REQ"));
					...
					PreparedQuery zp01AddrQuery = new PreparedQuery(entityManager,
							ExternalizedQuery.getSql("BATCH.GET_ADDR_ENTITY_CREATE_REQ"));
					...
					PreparedQuery newAddrQuery = new PreparedQuery(entityManager,
							ExternalizedQuery.getSql("BATCH.GET_ADDR_FOR_SAP_NO"));
	...
	public void processMassChanges(EntityManager entityManager, Admin admin, Data data)
		...
		if (input.getInputReqType() != null
				&& (input.getInputReqType().equalsIgnoreCase("D") || input.getInputReqType().equalsIgnoreCase("R"))) {
		...
		LOG.info("Sending request to Mass Update Service [Request ID: " + request.getReqId() + "  Type: "
				+ request.getReqType() + "]");
		...
		if (applicationId == null) {
			...
			response.setMsg("No application ID defined for Country: " + data.getCmrIssuingCntry()
		...
		LOG.info("Response received from Process Service [Request ID: " + response.getReqId() + " Status: "
				+ response.getStatus() + " Message: " + (response.getMsg() != null ? response.getMsg() : "-") + "]");
		...
		try {
			...
			for (ResponseRecord record : response.getRecords()) {
				PreparedQuery updtQuery = new PreparedQuery(entityManager,
				...
				for (MassUpdt massUpdt : updateList) {
					...
					LOG.info("Mass Update Record Updated [Request ID: " + massUpdt.getId().getParReqId() + " CMR_NO: "
							+ massUpdt.getCmrNo() + " SEQ No: " + massUpdt.getId().getSeqNo() + "]");
			...
			if (isCompletedSuccessfully(resultCode)) {
				...
				if (CmrConstants.RDC_STATUS_ABORTED.equals(resultCode)
						&& CmrConstants.RDC_STATUS_ABORTED.equals(processingStatus)) {
					...
					comment = comment.append(
							requestType + " in RDc aborted: " + response.getMsg() + "\nSystem will retry once.");
			...
			if (!CmrConstants.RDC_STATUS_IGNORED.equals(resultCode)) {
				RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin,
						comment.toString().trim(), action, null, null, "COM".equals(admin.getReqStatus()));
			}
			RequestUtils.createCommentLogFromBatch(entityManager, BATCH_USER_ID, admin.getId().getReqId(),
			...
			updateEntity(admin, entityManager);
			LOG.debug("Request ID " + admin.getId().getReqId() + " Status: " + admin.getRdcProcessingStatus()
					+ " Message: " + admin.getRdcProcessingMsg());
	...
	public void processMassChangesMulti(EntityManager entityManager, Admin admin, Data data)
		...
		for (USMassUpdateWorker worker : workers) {
			if (worker != null) {
				...
				if (CmrConstants.RDC_STATUS_ABORTED.equals(status)
						|| CmrConstants.RDC_STATUS_NOT_COMPLETED.equals(status)) {
					response.setStatus(CmrConstants.RDC_STATUS_ABORTED);
	...
	public void processUpdateByEnterprise(EntityManager entityManager, Admin admin, Data data)
		...
		try {
			...
			LOG.info("Processing Enterprise Record " + admin.getId().getReqId() + " [Request ID: "
					+ admin.getId().getReqId() + "]");

			PreparedQuery query = new PreparedQuery(entityManager,
					ExternalizedQuery.getSql("BATCH.MASS_UPDATE_DATA_RECORDS"));
			query.setParameter("REQ_ID", admin.getId().getReqId());
			...
			MassUpdtData enterpriseData = results != null && results.size() > 0
					? results.get(0).getEntity(MassUpdtData.class) : null;
			if (enterpriseData == null) {
				LOG.warn("Mass Update Data cannot be retrieved for Request ID " + admin.getId().getReqId()
						+ " Iteration " + admin.getIterationId());
			} else {
				EnterpriseUpdtRequest enterpriseUpdtRequest = prepareUpdtByEnterpriseRequest(entityManager, admin, data,
						enterpriseData);
				...
				LOG.info("Response received from Enterprise Service [Request ID: " + response.getReqId() + " Status: "
				...
				if (isCompletedSuccessfully(resultCode)) {
					...
					if (CmrConstants.RDC_STATUS_ABORTED.equals(resultCode)
							&& CmrConstants.RDC_STATUS_ABORTED.equals(currentProcessingStatus)) {
				...
				if (!CmrConstants.RDC_STATUS_IGNORED.equals(resultCode)) {
					RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin,
							comment.toString().trim(), ACTION_RDC_UPDT_BY_ENT, null, null,
			...
			LOG.error("Error in processing Enterprise Record " + admin.getId().getReqId() + " for Request ID "
					+ admin.getId().getReqId(), e);
	 ...
	 * @throws Exception
	 */
	protected void processCreateRequest(EntityManager entityManager, ProcessRequest request, Admin admin, Data data)
			throws Exception {
		...
		LOG.info("Sending request to Process Service [Request ID: " + request.getReqId() + " CMR No: "
				+ request.getCmrNo() + " Type: " + request.getReqType() + "]");
		...
		if (applicationId == null) {
			...
			response.setMandt(request.getMandt());
			response.setStatus(CmrConstants.RDC_STATUS_NOT_COMPLETED);
			response.setMessage("No application ID defined for Country: " + data.getCmrIssuingCntry()
					+ ". Cannot process RDc records.");
		} else {
			try {
		...
		LOG.info("Response received from Process Service [Request ID: " + response.getReqId() + " CMR No: "
				+ response.getCmrNo() + " Status: " + response.getStatus() + " Message: "
				+ (response.getMessage() != null ? response.getMessage() : "-") + "]");
		...
		try {
			...
			if (isCompletedSuccessfully(resultCode)) {
				...
				if (response.isCmrNoGenerated() && !StringUtils.isEmpty(response.getCmrNo())) {
					LOG.debug("CMR No. " + response.getCmrNo() + " generated. Updating DATA for Request "
							+ data.getId().getReqId());
				...
				if (response.getRecords() != null) {
					...
					if (prospectConversion) {
						comment = comment.append(
								"RDc processing converted prospect " + request.getProspectCMRNo() + " to KUNNR(s): ");
					} else {
				...
				if (CmrConstants.RDC_STATUS_ABORTED.equals(resultCode)
						&& CmrConstants.RDC_STATUS_ABORTED.equals(processingStatus)) {
					comment = comment.append("RDc create processing for CMR No " + request.getCmrNo()
			...
			/*
			 * Defect 1450224 - TransCon batch issue when writing Tax Separation
			 * Indc on CROS
			 */
			...
			if ((SystemLocation.UNITED_STATES.equals(data.getCmrIssuingCntry()) || isNotInternalAndChile(data, admin)
					|| isInternalAndSSAMX(data, admin))
					&& !CmrConstants.RDC_STATUS_ABORTED.equals(admin.getRdcProcessingStatus())
					&& CmrConstants.REQUEST_STATUS.COM.toString().equals(admin.getReqStatus())
					&& !"Y".equals(admin.getDisableAutoProc())) {
	...
	protected void updateRequestAddress(EntityManager entityManager, Admin admin, Data data, RDcRecord record) {
		...
		if ("897".equals(data.getCmrIssuingCntry())) {
			if ("ZP01".equals(record.getAddressType()) && record.getSeqNo() != null
					&& Integer.parseInt(record.getSeqNo()) >= 200) {
				...
				// Update
				// the ZI01 address
				addrQuery.setParameter("ADDR_TYPE",
						"ZS01".equals(record.getAddressType()) || "ZI01".equals(record.getAddressType()) ? "ZS01"
								: "ZI01");
			}
		} else {
		...
		for (Addr addr : addrList) {
			...
			LOG.info("Address Record Updated [Request ID: " + addr.getId().getReqId() + " Type: "
					+ addr.getId().getAddrType() + " SAP No: " + record.getSapNo() + "]");
			updateEntity(addr, entityManager);
		}
	}
	 ...
	 * @throws CmrException
	 */
	protected void handleMailsForResult(EntityManager entityManager, String resultCode, Admin admin, String comment)
			throws CmrException, SQLException {
		RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin, comment, ACTION_RDC_CREATE,
	...
	protected boolean isInternalAndSSAMX(Data data, Admin admin) {
		...
		if ((LAHandler.isSSAIssuingCountry(data.getCmrIssuingCntry())
				|| LAHandler.isMXIssuingCountry(data.getCmrIssuingCntry()))
				&& CmrConstants.SSAMX_INTERNAL_TYPES.contains(admin.getCustType())) {
			ret = true;
		}
	 ...
	 * @throws Exception
	 */
	protected void processUpdateRequest(EntityManager entityManager, ProcessRequest request, Admin admin, Data data)
			throws Exception {

		String resultCode = null;
		String processingStatus = admin.getRdcProcessingStatus() != null ? admin.getRdcProcessingStatus() : "";
		long reqId = admin.getId().getReqId();
		RequestChangeContainer changes = new RequestChangeContainer(entityManager, data.getCmrIssuingCntry(), admin,
				reqId);
		...
		try {
			...
			for (Addr addr : addresses) {
				...
				if (continueUpdate) {
					...
					LOG.info("Response received from Process Service [Request ID: " + response.getReqId() + " CMR No: "
							+ response.getCmrNo() + " KUNNR: " + addr.getSapNo() + " Status: " + response.getStatus()
					...
					if (isCompletedSuccessfully(resultCode)) {
						...
						if (response.getRecords() != null) {
							...
							if (CmrConstants.RDC_STATUS_COMPLETED_WITH_WARNINGS.equals(resultCode)) {
								comment = comment.append("\nWarning Message: " + response.getMessage());
								rdcProcessingMsg.append(
										"Warning: " + response.getMessage() + "[KUNNR:" + addr.getSapNo() + "]\n");
							}
						} else {
							comment.append("RDc records were not processed.");
							if (CmrConstants.RDC_STATUS_COMPLETED_WITH_WARNINGS.equals(resultCode)) {
						...
						if (CmrConstants.RDC_STATUS_ABORTED.equals(resultCode)
							...
							comment = comment.append("\nRDc update processing for KUNNR "
									+ (request.getSapNo() != null ? request.getSapNo() : "(not generated)")
									+ " failed. Error: " + response.getMessage() + "\n");
						} else if (CmrConstants.RDC_STATUS_ABORTED.equalsIgnoreCase(resultCode)) {
							comment = comment.append("\nRDc update processing for KUNNR "
									+ (request.getSapNo() != null ? request.getSapNo() : "(not generated)")
						...
						rdcProcessingMsg
								.append("Error: " + response.getMessage() + "[KUNNR:" + addr.getSapNo() + "]\n");
					}
			...
			if (statusCodes.contains(CmrConstants.RDC_STATUS_NOT_COMPLETED)) {
				...
				admin.setRdcProcessingStatus(processingStatus.equals(CmrConstants.RDC_STATUS_ABORTED)
						? CmrConstants.RDC_STATUS_NOT_COMPLETED : CmrConstants.RDC_STATUS_ABORTED);
			...
			if ("N".equals(admin.getRdcProcessingStatus()) || "A".equals(admin.getRdcProcessingStatus())) {
				rdcProcessingMsg = new StringBuilder(
						"Some errors occurred during processing. Please check request's comment log for details.");
			} else {
				rdcProcessingMsg = new StringBuilder(
						"Processing has been completed. Please check request's comment log for details.");
			}
			...
			if ("N".equals(admin.getRdcProcessingStatus()) || "A".equals(admin.getRdcProcessingStatus())) {
				RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin,
						"Some errors occurred during processing. Please check request's comment log for details.",
						ACTION_RDC_UPDATE, null, null, "COM".equals(admin.getReqStatus()));
			} else {
				if (!ignoreWfhistory) {
					RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin,
							"Processing has been completed. Please check request's comment log for details.",
	...
	protected void forceChangeToUpdate(EntityManager entityManager, Admin admin, Data data, boolean oneTaxInfoNotEmpty)
		...
		if (SystemLocation.UNITED_STATES.equals(issuingCntry) && !StringUtils.isBlank(data.getCmrNo())
				&& (!StringUtils.isBlank(data.getAffiliate()) || !StringUtils.isBlank(data.getEnterprise())
						|| !StringUtils.isBlank(data.getIccTaxClass())
						|| !StringUtils.isBlank(data.getIccTaxExemptStatus())
						|| !StringUtils.isBlank(data.getNonIbmCompanyInd()) || !StringUtils.isBlank(data.getUsSicmen())
						|| !StringUtils.isBlank(data.getSpecialTaxCd()) || isT2PoolCMRRequest(admin, data))) {
			...
			RequestUtils.createWorkflowHistoryFromBatch(entityManager, BATCH_USER_ID, admin, FORCED_UPDATE_COMMENT,
					FORCED_CHANGE_ACTION, null, null, "COM".equals(admin.getReqStatus()));
	...
	protected boolean isT2PoolCMRRequest(Admin admin, Data data) {
		if (USUtil.BUSINESS_PARTNER.equals(admin.getCustType()) && "TT2".equals(data.getCsoSite())
				&& "P".equals(data.getBpAcctTyp()) && "IRMR".equals(data.getBpName())) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Reverts the request to a 'Create' if it detects that the system forced
	 * the update
	 * 
	 * @param entityManager
	 * @param reqId
	 ...
	 * @throws Exception
	 */
	protected void convertToProspectToLegalCMRInput(ProcessRequest request, EntityManager entityManager, long reqId)
			throws Exception {
		...
		if (results != null && results.size() > 0) {
			...
			if (CmrConstants.YES_NO.Y.toString().equals(result[0])) {
				...
				if (StringUtils.isEmpty(prospectCMR)) {
					throw new Exception("Cannot process Propsect to Legal CMR conversion for Request ID " + reqId
							+ ". Propsect CMR No. is missing.");
				}
	 ...
	 * @throws SQLException
	 */
	public void createNotifyReqCommentLog(EntityManager entityManager, NotifyReqModel model)
			throws CmrException, SQLException {
	 ...
	 * @throws SQLException
	 */
	public void createMQIntfReqCommentLog(EntityManager entityManager, MQIntfReqQueueModel model)
			throws CmrException, SQLException {
		...
		if (!StringUtils.isBlank(model.getExecpMessage())) {
			MQProcessingException exception = MQProcessingException.parse(model.getTargetSys(),
					model.getExecpMessage());
			log.setCmt(exception.toString(2000, MQErrorLine.TYPE_MANDATORY_ERROR, MQErrorLine.TYPE_GENERAL_ERROR,
					MQErrorLine.TYPE_ANY_ERROR));
		} else {
			log.setCmt("");
		}
		// save cmtlockedIn as Y default for current realese
	...
	protected void initClient() throws Exception {
		...
		if (this.massServiceClient == null) {
			this.massServiceClient = MassServicesFactory.getInstance().createClient(BATCH_SERVICES_URL,
					MassProcessClient.class);
		}
		if (this.updtByEntClient == null) {
			this.updtByEntClient = CmrServicesFactory.getInstance().createClient(BATCH_SERVICES_URL,
					UpdateByEntClient.class);
		}
	}
	...
	public MassProcessRequest prepareMassUpdateRequest(EntityManager entityManager, Admin admin, Data data,
			MassUpdateServiceInput massUpdateServiceInput)
			throws JsonGenerationException, JsonMappingException, IOException, Exception {
		MassProcessRequest request = new MassProcessRequest();
	 ...
	 */
	protected void fetchMassUpdateDataDetails(EntityManager entityManager, Admin admin, Data data,
			List<MassUpdateRecord> updtRecordsList) {
		long reqId = admin.getId().getReqId();
	 ...
	 * @param updtRecordsList
	 */
	protected void fetchMassUpdateAddressDetails(EntityManager entityManager, Admin admin, Data data,
			List<MassUpdateRecord> updtRecordsList) {
		...
		PreparedQuery query = new PreparedQuery(entityManager,
				ExternalizedQuery.getSql("BATCH.MASS_UPDATE_ADDR_RECORDS"));
		query.setParameter("REQ_ID", reqId);
		query.setParameter("ITERATION_ID", itrId);
		query.setParameter("STATUS", "PASS");
	 ...
	 * @throws Exception
	 */
	public MassProcessRequest prepareReactivateDelRequest(EntityManager entityManager, Admin admin, Data data,
			MassUpdateServiceInput input) throws JsonGenerationException, JsonMappingException, IOException, Exception {
	 ...
	 * @throws Exception
	 */
	public EnterpriseUpdtRequest prepareUpdtByEnterpriseRequest(EntityManager em, Admin admin, Data data,
			MassUpdtData massUpdtData) throws JsonGenerationException, JsonMappingException, IOException, Exception {
	...
	protected boolean isCompletedSuccessfully(String status) {
		return CmrConstants.RDC_STATUS_COMPLETED.equals(status)
				|| CmrConstants.RDC_STATUS_COMPLETED_WITH_WARNINGS.equals(status);
	}
	...
	protected void deleteRDcTargetRecords(EntityManager entityManager, String cmrNo, String country) {
		try {
			...
			if (!kunnrs.isEmpty()) {
				...
				IndexServiceClient client = new IndexServiceClient(
						SystemConfiguration.getValue("BATCH_CI_SERVICES_URL"));
